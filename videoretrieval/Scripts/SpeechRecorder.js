 ...
 * 1.0.8.1   2013/12/16   删除IAT_KEY事件，在IAT_COMPLETE中获取key
 * 1.0.9.0   2013/12/26   增加getLog和clearLog方法
 */
...
(function (win) {
    ...
    /**
     * SpeechRecorder事件定义
     * @type {{READY: string, ERROR: string, RECORD_STATE: string, EVAL_RESULT: string, RECORDING: string, MICROPHONE_STATE: string, PLAYING: string, PLAY_STATE: string, AUDIO_LOADED: string, AUDIO_CHECK_RESULT: string, PLUGIN_LOAD: string, IAT_RESULT: string, IAT_COMPLETE: string, SYNTH_PROGRESS: string, SYNTH_RESULT: string, DOWNLOAD_PROGRESS：string}}
     * @enum
     */
    ...
    SpeechRecorderObject.prototype = {
        ...
        getLog: function (type) {
            return this.elemObj().getLog(type);
        },

        clearLog: function(type){
            return this.elemObj().clearLog(type);
        }
    };
