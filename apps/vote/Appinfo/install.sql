...
CREATE TABLE `ts_vote` (
  ...
  `creator_login_name` varchar(100) NOT NULL,
  `creator_user_name` varchar(100) NOT NULL,
  `title` text NOT NULL COMMENT '标题',
  `explain` text NOT NULL COMMENT '介绍',
  ...
  `isHot` varchar(1) NOT NULL COMMENT '是否热门',
  `rTime` int(11) NOT NULL COMMENT '访问时间',
  `status` varchar(1) NOT NULL COMMENT '是否禁用:0禁用,1激活',
  ...
  PRIMARY KEY (`id`),
  KEY `LOGIN_NAME_INDEX` (`creator_login_name`)
...
CREATE TABLE `ts_vote_user` (
  ...
  `opts` text NOT NULL COMMENT '用户所投选项',
  `cTime` int(11) DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
VALUES
	('', 'add_vote', '发起调研', 'vote', '{action}{sign}了{score}{typecn}', '20', '20'),
	('', 'join_vote', '参与调研', 'vote', '{action}{sign}了{score}{typecn}', '1', '5'),
	('', 'joined_vote', '调研被参与', 'vote', '{action}{sign}了{score}{typecn}', '1', '1'),
	('', 'delete_vote', '删除调研', 'vote', '{action}{sign}了{score}{typecn}', '-20', '-20');
...
INSERT INTO `ts_lang` (`key`,`appname`,`filetype`,`zh-cn`,`en`,`zh-tw`) VALUES 
 ...
 ('VOTE_SETTING', 'VOTE', '0', '调研设置', 'The vote is set', ''),
 ('OPTION', 'VOTE', '0', '候选项', 'The candidate item', ''),
 ('VOTE_TOPIC', 'VOTE', '0', '调研主题', 'Voting theme', ''),
 ('PUBLIC_APPNAME_VOTE', 'PUBLIC', '0', '网络调研', 'Vote', '');

 DELETE FROM `ts_lang` WHERE `key` = 'PUBLIC_APPNAME_VOTE';
INSERT INTO `ts_lang` (`key`,`appname`,`filetype`,`zh-cn`,`en`,`zh-tw`) VALUES ('PUBLIC_APPNAME_VOTE', 'PUBLIC', '0', '网络调研', 'Vote', '');
