...
class VoteModel extends BaseModel{
     ...
     * @return void
     */
    public function addVote($data, $opt, $tags, $gids,$opt_origin){
        ...
        if($vote_id){
        	...
        	$optDao = D("VoteOpt");
        	foreach($opt as $k=>$v) {
        		foreach($opt_origin as $j=>$v1){
        			if($k==$j) {
        				$data["vote_id"] = $vote_id;
        				$data["name"] = t($v);
        				$data["name_origin"] = t($v1);
        				$add = $optDao->add($data);
        			}
        		}
        	}
     ...
     * @return void
     */
    public function synUserVote($vote_id, $data, $tags, $gids){
    	$invitees = $data['invitees'];//被邀请者id
    ...
    if ($vote_id) {
        	...
        	//add by yxxing 产生网络调研动态
        	$viewUrl = U("vote/Index/detail",array("id"=>$vote_id));
    ...
    public function getVotesBycondition($type=0,$condition,$order = 'id DESC',$limit = 20){
        switch ($type) {
            ...
            case 2:
                ...
                $join=' LEFT JOIN '.$this->tablePrefix.'vote_user vu ON vu.vote_id = v.id';
     ...
     * 调研中心，查询操作
     * 
     * 最新调研：$order = 'cTime DESC'
     * 最热调研：$order = 'vote_num DESC'
     *
     */
    ...
    public function doDeleteVote($id){
        ...
        //删除调研回复
        $result4 = $votePost->where($map2)->save(array('is_del'=>1));
         ...
         */
		 ...
		 public function geAppHotVote(){
		 	...
		 	foreach ($hot_list as $key){
		 		...
		 		$map['vote_id']=$key['id'];
		 		//网络调研精彩评论获取
		 		$comment =D('VotePost','vote')->where($map)->order('agree_count DESC, comment_count DESC, ctime DESC')->find();
		 		//网络调研发表者信息
		 		...
		 		$blogInfo['id']=$key['id'];
		 		$blogInfo['content'] =htmlentities($key['explain']);
		 ...
		 private function _syncToFeed($uid, $viewUrl, $title, $content, $gids, $toSpace=1) {
             ...
             $d['body'] = '我发起了网络调研【'.getShort($title,20).'】'.getShort($content,30).'&nbsp;';
             $feed = model('Feed')->put($uid, 'vote', "vote", $d);
             if(!empty($gids)){
                 foreach ($gids as $gid) {
                     if(!empty($gid)){
        		 		$d['gid'] = $gid;
        		 		$d['body'] = "@" . $GLOBALS['ts']['user']['uname'] . ' 我发起了网络调研【'.getShort($title,20).'】'.getShort($content,30).'&nbsp;';
