...
class FeedAction extends Action {
	...
	public function PostFeed(){
		...
		//录音ID
		$d['record_id'] = $_POST['record_id'];
		//标题
		$d['feed_title'] = $_POST['feed_title'];
		// 附件信息
        $d['attach_id'] = preg_replace("/[^\w|,|\|]/","",trim(t($_POST['attach_id']), "|"));
		if ( !empty($d['attach_id']) ){
			$d['attach_id'] = explode('|', $d['attach_id']);
			array_map( 'intval' , $d['attach_id'] );
		}
		//添加班级id
		$class_id = isset($_POST['class_id']) ? t($_POST['class_id']) : "";
		!empty($class_id) && $d['class_id'] = $class_id;
		
		//名师工作室id
		$gid = isset($_POST['gid']) ? t($_POST['gid']) : "";
		!empty($gid) && $d['gid'] = $gid;
		// 发送微博的类型
		$type = t($_POST['type']);
		// 所属应用名称
		$app = isset($_POST['app_name']) ? t($_POST['app_name']) : APP_NAME;			// 当前动态产生所属的应用
		//下面这一步model(Feed)->put 同步发送消息比较耗时 By Zhangce 2017/1/21
        if(!$data = model('Feed')->put($this->uid, $app, $type, $d)) {
			$return = array('status'=>0,'data'=>model('Feed')->getError());
			exit(json_encode($return));
		}
		//是否是日志转发
		$blog_id = isset($_POST['blog_id']) ? t($_POST['blog_id']) : "";
        //统计记录
        $this->operationLog["actionName"]="feedTalk";
        $this->operationLog["remark"]="个人空间发表说说";
        ...
        $conmment = $this->user['uname']."于".date("Y-m-d H:i:s",time())."在".C("appId")[$addr]['name'].$op."了“".$d['body']."”的".$actionName;
        $logObj = $this->getLogObj(C("appType")["hdjl"]["code"],C("appId")[$addr]["code"],C("opType")["share"]['code'],$return['feedId'],C("location")["localServer"]["code"],"","",$d['body'],$this->mid,$this->user['login'],$this->user['uname'],$this->roleEnName,"",$conmment);
	...
	public function removeFeed() {
		...
		//更新相关信息到solr服务器
		$userrole = model('UserRole')->getUserRole($this->mid);
		$roletype = $userrole['rolename'];
		in_array($roletype,array(UserRoleTypeModel::PROVINCE_RESAERCHER,UserRoleTypeModel::CITY_RESAERCHER,UserRoleTypeModel::COUNTY_RESAERCHER,UserRoleTypeModel::RESAERCHER))  && $type = 'researcher';
		switch ($roletype){
			case 'teacher':
				$roletype = ZoneTypeModel::TEACHER;
				break;
			case 'researcher':
				$roletype = ZoneTypeModel::RESEARCHER;
				break;
		}
		$userData = model('UserData')->getUserData($this->mid);//获取当前登录者发布微博总数//获取当前登录者发布微博总数
		$return['status'] == 1;
		// 删除频道关联信息
	...
	public function addComment () {
		...
		if ($data ['comment_id']) {
			...
			<dt><a href="'.$commentInfo['user_info']['space_url'].'"><img src="'.$commentInfo['user_info']['avatar_small'].'" width="30" height="30"/></a></dt>
			<dd>
	...
	public function addReport () {
		...
		if($return['status'] == 1) {
			...
			<dt><a href="'.$return['data']['user_info']['space_url'].'"><img src="'.$return['data']['user_info']['avatar_small'].'" width="30" height="30"/></a></dt>
			<dd>
