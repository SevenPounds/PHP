...
class MessageAction extends Action {
	...
	public function notify() {
		$list = D('notify_message')->where('uid='.$this->mid.' AND type=0 ')->order('ctime desc')->findpage(20);
	...
	public function digg(){
		$list = D('notify_message')->where('uid='.$this->mid.' AND type=1')->order('ctime desc')->findpage(20);
		foreach($list['data'] as $k=>$v){
			if($appname !='public'){
				$list['data'][$k]['app'] = model('App')->getAppByName($v['appname']);
			}
		...
		model('Notify')->setRead($this->mid,'',1);
		
		$this->assign('list',$list);
	
		$this->setTitle( L('PUBLIC_MESSAGE_DIGG') );
		$this->setKeywords( L('PUBLIC_MESSAGE_DIGG') );
		$this->display('mydigg');
	}
	...
	public function doPost() {
		...
		$_POST["content_origin"] = $_POST["content"];
		$sensitiveWord = $this->sensitiveWord_svc;
		$contentReplace = $sensitiveWord->checkSensitiveWord($_POST["content"]);
		$contentReplace = json_decode($contentReplace,true);
		if($contentReplace["Code"]!=0){
			return;
		}
		$_POST["content"] = $contentReplace["Data"];

		$res = model('Message')->postMessage($_POST, $this->mid);
        $targetUser =  M("User")->getUserInfo($_POST['to']);
		if ($res) {

            $conmment = $this->user['uname']."于".date("Y-m-d H:i:s",time())."给".$targetUser['uname']."发送了“".$_POST["content_origin"]."”的私信";
            $logObj = $this->getLogObj(C("appType")["hdjl"]["code"],C("appId")["jssq"]["code"],C("opType")["create"]['code'],$res,C("location")["localServer"]["code"],"",$_POST['to'],$_POST['content_origin'],$this->mid,$this->user['login'],$this->user['uname'],$this->roleEnName,"",$conmment);
