<?php
    ...
    class BaseModel extends Model{
        ...
        public function getBlogContent( $id,$how =null,$condition = array()  ){
            ...
            isset( $condition['uid'] ) && $map['uid'] = $condition['uid'];
            isset( $condition['cid'] ) && $map['class_id'] = $condition['cid'];
            ...
            if( false == $result ){
                if( "gt" == $how ){
                    return $this->getBlogContent( $id,'first',$condition );
                }

                if( "lt" == $how ){
                    return $this->getBlogContent( $id,'last',$condition );
                }
                return false;
            }
            //清除data。防止污染
            ...
            //追加日志中提到的内容
            $result['count']    = $this->where( $where." AND status = 1 " )->count();
            $result['num']      = $this->where( 'id <'.$result['id']." AND status = 1 AND ".$where )->count()+1;
        ...
        public static function factoryModel( $name ){
            return D("Blog".ucfirst( $name ),'blog');
        }
