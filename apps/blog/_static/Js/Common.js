 ...
 * @param  资源名称
 */
function setShare(id,uid,blogTitle){ 
	//jiathis_config.url = encodeURIComponent( U('blog/Index/show') + "&id=" + id + "&mid=" + uid);
	jiathis_config.summary = "我在深圳教育云发现了一个很不错的日志：“"+blogTitle+"”，快来看看吧~";
};
...
function addCategory(name) {
    ...
    $.post( U('blog/Index/addCategory'),{name:name,cid:cid},function( txt ){
    	if(-1 == txt) {
    		ui.error( '添加失败' );
        }else if(-2 == txt) {
            ui.error( "分类名已存在" );
