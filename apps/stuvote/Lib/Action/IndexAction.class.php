...
class IndexAction extends AccessAction {
	...
	function add() {
		...
		$voteDao = D("Vote");
		try {
			$result = $voteDao->addVote($data, $opt, $tagIds, $gidsArr);
		} catch(ThinkException $e) {
			$this->error($e->getMessage());
		}

		if ($result) {
            //记录行为  2014-11-21  by nandeng
            setVoteActionRecord($this->mid,0,$result,0,'vote');
			model('Credit')->setUserCredit($this->mid,'add_vote');
			$this->assign('jumpUrl', U('stuvote/Index/detail', array('id'=>$result)));
			$this->ajaxData['url'] = U('stuvote/Index/detail', array('id'=>$result));
	...
	function detail() {
		...
		$vote = D("Vote")->find($id);
		if( false == $vote ) {
			$this->assign('jumpUrl', U('stuvote/Index/index'));
		...
		try {
			...
			Log::write('网络投票详情页埋点异常：'.$e->getMessage());
		}
	...
	function vote() {
        ...
        //记录行为  2014-11-21  by nandeng
        setVoteActionRecord($this->mid,$vote_user_id,$addid,$vote_id,'stuvote_opt');
        addVoteMessage($this->mid,$vote_user_id,$the_vote['title'],$vote_id,'stuvote_vote',$data['opts']);
		//投票选项信息的num+1
		$dao = D("VoteOpt");
    ...
    public function addAgree(){
        ...
        if(empty($id) || !is_numeric($id)){
        	exit ("{'status':'400','info':'参数不合法！'}");
        }
        $res = model('Agree')->addAgree($id, $this->mid);
        if($res){
    ...
    public function sendMsg(){
        ...
        $vote = D('vote')->where('id='.$vote_id)->find();
        addVoteMessage($this->mid,$vote['uid'],$vote['title'],$vote_id,'vote_post',$content);
    }
}
?>
