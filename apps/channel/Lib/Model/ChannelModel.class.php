...
class ChannelModel extends Model{
	...
	public function getDataWithCid($cid, $loadId, $limit = 20){
		...
		$map['b.is_del']=$countmap['b.is_del']=0;
		!empty($cid) && $map['channel_category_id'] = $countmap['channel_category_id'] = $cid;
		$table="`{$this->tablePrefix}channel` AS a LEFT JOIN `{$this->tablePrefix}feed` AS b ON a.feed_id = b.feed_id";
		$result = $this->table($table)->where($countmap)->order('feed_channel_link_id DESC')->findPage($limit * 4);
		...
		$data = $this->table($table)->where($map)->order('feed_channel_link_id DESC')->limit($limit)->findAll();
		// 设置指定的宽高
	 ...
	 * @return 返回所有的feed_id
	 */
	public function getChannelFeedId($limit){
		$map['status']=1;
		$data = $this->where($map)->field('feed_id')->limit($limit)->select();
		return  $data;
	}
	/**
	 * 返回排序的feed_id
	 * @param 限制数目 $limit
	 */
	public function getOrderedFeedId($limit){
		$sql='SELECT ts_ids.feed_id FROM ( SELECT DISTINCT feed_id FROM ts_channel ) AS ts_ids JOIN ts_collection AS tsc ON tsc.source_id = ts_ids.feed_id LEFT JOIN ts_feed AS tf ON ts_ids.feed_id = tf.feed_id where tf.is_del = 0 GROUP BY tsc.source_id ORDER BY count(tsc.source_id) DESC limit '.$limit;
		$feedList = $this->query($sql);
		$feedIds = getSubByKey($feedList, 'feed_id');
		return $feedIds;
	}
	/**
	 * 根据该栏目下记录的数量获取最热门的栏目
	 * @param 限制数目 $limit
	 */
	public function getHotCategory($limit){
		$map='channel_category_id > 12';
		$data=$this->field('count(channel_category_id) as cutnum,channel_category_id')->where($map)->group('channel_category_id')->order('cutnum DESC')->limit($limit)->select();
		$feedIds = getSubByKey($data, 'channel_category_id');
		return $feedIds;
	}
}