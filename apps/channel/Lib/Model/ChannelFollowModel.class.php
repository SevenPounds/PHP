...
class ChannelFollowModel extends Model{
    ...
    public function upFollow2($uid, $cid,$type=1){
    	// 验证数据的正确性
    	$map['uid'] = $uid;
    	if($type==2){
    		$result2 = $this->where($map)->delete();
    		return $result2;
    	}
    	if(empty($uid) || empty($cid)) {
    		return false;
    	...
    	foreach($cid as $key=>$value){
    		$map['channel_category_id'] = $value;
    		$isExist = $this->where($map)->count();
    		if($isExist == 0) {
    			$data['uid'] = $uid;
    			$data['channel_category_id'] = $value;
    			$result = $this->add($data);
    			$result = (boolean)$result;
    		}
    	}
    	$map2['uid'] = $uid;
    	$map2['channel_category_id'] = array('not in',$cid);
    	$result2 = $this->where($map2)->delete();
	...
	public function getFollowList($uid){
		...
		$list = D()->table("`".C('DB_PREFIX')."channel_follow` AS f LEFT JOIN `".C('DB_PREFIX')."channel_category` AS c ON f.channel_category_id=c.channel_category_id")
				   ->field('c.`channel_category_id`, c.`title`,c.`pid`')
				   ->where($map)
				   ->findAll();
	 ...
	 * 获取用户关注栏目的id
	 * @param int $uid
	 * @return array 用户关注栏目id
	 */
    public function getFollowChannelId($uid){
    	if(empty($uid)){
    		return array();
    	}
    	$map['f.uid'] = $uid;
		$list = D()->table("`".C('DB_PREFIX')."channel_follow` AS f LEFT JOIN `".C('DB_PREFIX')."channel_category` AS c ON f.channel_category_id=c.channel_category_id")
				   ...
				   ->where($map)
				   ->findAll();
		return getSubByKey($list,'channel_category_id');
    }
	/**
	 ...
	 */
     public function getFollowingFeed($where = '', $limit = 10, $uid = '', $fgid = ''){
		...
		$feedList = D()->table($table)->field(' DISTINCT a.feed_id')->where($where)->order('c.publish_time DESC')->findPage($limit);
	 ...
	 * @param integer $limit 返回结果集的数目,默认为10
	 * @param integer $uid 指定用户的 id
	 * @param integer $fgid 用户关注的分组的id
	 * @return 返回知识堂所有的微博
	 */
	public function getFollowingFeed2($where = '',$limit = 10, $uid = '', $fgid = ''){
	
		$isfollow=intval($this->where('uid='.$GLOBALS['ts']['mid'])->count());
		if($isfollow){
			$buid = empty($uid) ? $GLOBALS['ts']['mid'] : $uid;
			...
			$feedList=$this->getAllChannelFeed($where,$limit,'','');
		}
		return $feedList;
	}
	/**
	 ...
	 * @return 返回知识堂所有的微博
	 */
    public function getAllChannelFeed($where = '',$limit = 10, $uid = '', $fgid = ''){
		$where .= " AND a.status=1";
		$table="`{$this->tablePrefix}channel` AS a LEFT JOIN `{$this->tablePrefix}feed` AS b ON a.feed_id = b.feed_id";
		!empty($fgid) && $where .= ' AND b.channel_category_id = '.$fgid;
		$feedList=D()->table($table)->field(' DISTINCT a.feed_id ')->where($where)->order('a.feed_id DESC')->findPage($limit);
		$feedIds = getSubByKey($feedList['data'], 'feed_id');
		$feedList['data'] = model('Feed')->getFeeds($feedIds);
		return $feedList;
	}
