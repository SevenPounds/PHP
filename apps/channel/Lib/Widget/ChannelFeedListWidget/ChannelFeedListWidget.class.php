 ...
 * @version TS3.0
 */
class ChannelFeedListWidget extends Widget {
	...
	public function render($data) {
        ...
        // 查询是否有话题ID
        if($var['topic_id']) {
        	$content = $this->getTopicData($var,'_ChannelFeedList.html');
        } else {
        	$content = $this->getData($var,'_ChannelFeedList.html');
        }
        // 查看是否有更多数据
    ...
    public function loadMore() {
        ...
        if($_REQUEST['topic_id']) { 
            $content = $this->getTopicData($_REQUEST,'_ChannelFeedList.html');
        } else {
    	    $content = $this->getData($_REQUEST,'_ChannelFeedList.html');
        }
        // 查看是否有更多数据	
    	if(empty($content['html'])) {
            // 没有更多的
    	...
    	switch($type) {
            ...
            case 'channel':
                ...
                foreach($data as &$value){
                	...
                	$value['publish_time'] = date('Y-m-d H:s', $value['publish_time']);
                }
                $list['data']=$data;
                unset($data);
                break;
            case 'channelall':
            	$where = ' (b.is_audit=1 OR b.is_audit=0) AND b.is_del = 0 ';
                if($var['loadId'] > 0) { //非第一次
                    $where .= " AND b.feed_id < '".intval($var['loadId'])."'";
                }
                if(!empty($var['feed_type'])) {
                    $where .= " AND b.type = '".t($var['feed_type'])."'";
                }

                $list = D('ChannelFollow', 'channel')->getAllChannelFeed($where, $this->limitnums, '' ,$var['fgid']);
                $data=$list['data'];
                foreach($data as &$value){
                	$value['feed_data']=unserialize($value['feed_data']);
                	$value['publish_time'] = date('Y-m-d H:s', $value['publish_time']);
