...
class ContentWidget extends Widget{
	...
	public function render($data){
		...
		if($template === 'load') {
			$var['categoryJson'] = json_encode($data['channelCategoryall']);
		}
    ...
    public function loadMore(){
    	...
    	// 获取HTML数据
    	if($cid==-1){
    		$content = $this->getData($cid, $loadLimit, $loadId,'loadshare');
    	}else{
    ...
    public function getData($cid, $loadLimit, $loadId,$type='') {
		// 获取微博数据
		if(!empty($type) && $type=='loadshare'){
			$list['data']= D('ChannelFollow', 'channel')->getFollowingFeed($where,$loadLimit,'','');
			$var['data']=$list['data'];
		}else{
    	...
    	if(!empty($type) && $type=='loadshare'){
    		$content['html'] = fetch(dirname(__FILE__).'/loadshare.html', $var);
    	}else{
