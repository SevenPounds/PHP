<feed app='public' type='repost' info='转发微博'>
	...
	<body>
		...
		<dl class="comment">
			...
			<p class="info">
				<span class="right">
					<a href="{:U('group/Group/detail',array('uid'=>$sourceInfo['uid'],'feed_id'=>$sourceInfo['feed_id'],'gid'=>$sourceInfo['gid']))}">原文转发<neq name="sourceInfo.repost_count" value="0">({$sourceInfo.repost_count})</neq></a><i class="vline">|</i>
					<a href="{:U('group/Group/detail',array('uid'=>$sourceInfo['uid'],'feed_id'=>$sourceInfo['feed_id'],'gid'=>$sourceInfo['gid']))}">原文评论<neq name="sourceInfo.comment_count" value="0">({$sourceInfo.comment_count})</neq></a>
				</span>
				<span><a href="{:U('group/Group/detail',array('uid'=>$sourceInfo['uid'],'feed_id'=>$sourceInfo['feed_id'],'gid'=>$sourceInfo['gid']))}" class="date" date="{$sourceInfo['publish_time']}">{$sourceInfo['publish_time']|friendlyDate}</a><span>{:getFromClient($sourceInfo['from'])}</span></span>
			</p>
			<php>else:</php>
