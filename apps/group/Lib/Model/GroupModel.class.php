...
class GroupModel extends Model {
    ...
    public function getHotList($reset=false) {
    	...
    	if(!($cache = S('Cache_Group_Hot_list')) || $reset){
    		...
    		//只有有缓存时间且缓存不超过60秒的数据才可以返回，其他全部重新查询
    	...
    	for($i=0;$i<$cnt-1;$i++){//循环比较
    		for($j=$i+1;$j<$cnt;$j++){
    			if($hot_list[$j]['membercount']>$hot_list[$i]['membercount']){//执行交换
    				$temp=$hot_list[$i];
	 ...
	 * 获得最新的圈子
	 */
	function getNewList($reset=false){
		// 1分钟锁缓存
		if(!($cache = S('Cache_Group_New_list')) || $reset){
			S('Cache_Group_New_list_t',time()); //缓存未设置 先设置缓存设定时间
		}else{
			if(!($cacheSetTime =  S('Cache_Group_New_list_t')) || $cacheSetTime+60 <= time()){
				S('Cache_Group_New_list_t',time()); //缓存未设置 先设置缓存设定时间
			}else{
				return $cache;
			}
		}
		$new_list = $this->field('id,id AS gid,name,logo,membercount,ctime')->where('status=1 AND is_del=0 ')->order('ctime desc')->findAll();
		$data = array_slice($new_list, 0, 10);
		S('Cache_Group_New_list',$data);
		return $data;
	}
	/**
	 * 获得最热的圈子
	 */
	function getHotGroupList($reset=false){
		// 1分钟锁缓存
		if(!($cache = S('Cache_Group_HotDemo_list')) || $reset){
			S('Cache_Group_HotDemo_list',time()); //缓存未设置 先设置缓存设定时间
		}else{
			if(!($cacheSetTime =  S('Cache_Group_HotDemoT_list_t')) || $cacheSetTime+60 <= time()){
				S('Cache_Group_HotDemoT_list_t',time()); //缓存未设置 先设置缓存设定时间
			}else{
				return $cache;
			}
		}
		$new_list = $this->field('id,id AS gid,name,logo,membercount,ctime')->where('status=1 AND is_del=0 ')->order('membercount desc,ctime desc')->findAll();
		
		$data = array_slice($new_list, 0, 10);
		S('Cache_Group_HotDemo_list',$data);
		return $data;
	}
	/**
	 *  最新话题
	...
	function getnewtopic($uid) {
		return D('Topic')->getNewThread();
	}
	/**
	 * 最新话题
	 */
	function gethottopic(){
		return D('Topic')->getHotThread();
	}
	//获取群组动态
