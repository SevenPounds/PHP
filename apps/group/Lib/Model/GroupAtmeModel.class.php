...
class GroupAtmeModel extends Model {
	...
	private function _saveAtme($uids, $row_id, $gid) {
		foreach ($uids as $u_v) {
			...
			$data[] = "('{$this->_app}', '{$this->_app_table}', {$row_id}, {$gid}, {$u_v})";
            // 更新@Me的未读数目
            D('GroupUserCount')->addCount($u_v, 'atme', $gid ,1);
		}
        !empty($data) && $res = $this->execute('INSERT INTO '.$this->tablePrefix.'group_atme (`app`, `table`, `row_id`, `gid`, `uid`) VALUES '.implode(',', $data));
