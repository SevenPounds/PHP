...
class TopicModel extends Model {
    ...
    public function getHotThread(){
    	...
    	$monthday   = $today-24*3600*30*3;
    	// 缓存锁结束
    	$cache = $this->field('topic.id,topic.gid,topic.title,topic.dist,post.content,topic.name as author,topic.replytime as replyTime,topic.replycount as replyCount,topic.viewcount as viewCount')
		    ...
		    ->where('`group`.brower_level=-1 AND post.istopic = 1 AND topic.is_del=0 and post.is_del=0 AND topic.replytime>' . $monthday)
		    ->order('topic.replycount DESC')
		    ->limit(10)->findAll();
    ...
    public function getNewThread(){
    	//从配置中获取热门话题的天数
    	$fromTime=time()-C("HOT_TOPIC_TIME")*24*3600;
    	// 缓存锁结束
    	$cache = $this->field('topic.id,topic.gid,topic.title,topic.name as author,topic.replytime as replyTime,topic.dist,group.logo as logo,topic.addtime,post.content,topic.replycount as replyCount,topic.viewcount as viewCount')
	    	->table(C('DB_PREFIX').'group_topic as topic
				left join '.C('DB_PREFIX').'group_post as post
				on topic.id = post.tid
				left join ' . C('DB_PREFIX') . 'group as `group`
				on topic.gid=`group`.id')
		    ->where('`group`.brower_level=-1 AND post.istopic = 1 AND topic.is_del=0 and post.is_del=0 AND topic.replytime>' .$fromTime )
		    ->order('topic.addtime DESC')
		    ->limit(10)->findAll();
    	return $cache;
    }
