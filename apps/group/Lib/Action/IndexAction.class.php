...
class IndexAction extends BaseAction {
	...
	private function _indexRight(){
		...
		if(empty($catelist)) {
			...
			S('Category_pid',$catelist,3600);
		}

		$hotlist = D('Group', 'group')->getHotGroupList();
		$hostMaxs = array_slice($hotlist,0,1);
		$hostMax=$hostMaxs[0];
		$hostMax['logo'] = logo_path_to_url($hostMax['logo']);
		$this->assign('hostMax',$hostMax);
		$hotlist = array_slice($hotlist,1,count($hotlist)>6?6:count($hotlist));
		//名称截取
		foreach ( $hotlist as &$hv ) {
			$hv['short_name'] = getShort( $hv['name'] , 5);
			$hv['logo'] = logo_path_to_url( $hv['logo'] );
		}
		$new_list  = D('Group', 'group')->getNewList();//有缓存
		foreach ( $new_list as &$hv ) {
			$hv['short_name'] = getShort( $hv['name'] , 5);
			$hv['logo'] = logo_path_to_url( $hv['logo'] );
		}
		$this->assign('new_list',array_slice($new_list,0,count($new_list)>7?7:count($new_list)));
		$this->assign( 'hotlist' , $hotlist );
		
		$new_topics = D('Topic')->getNewThread();		
		foreach ( $new_topics as &$hv ) {
			$hv['logo'] = logo_path_to_url( $hv['logo'] );
		}
		$this->assign('topic_list',$new_topics);
	...
	public function doAdd()	{
		...
		if (trim($_POST['dosubmit'])) {
			...
			if($gid) {
                ...
                $conmment = $this->user['uname']."于".date("Y-m-d H:i:s",time())."在兴趣圈子创建了“".$group['name_origin']."”的兴趣圈子";
                $logObj = $this->getLogObj(C("appType")["hdjl"]["code"],C("appId")["xqqz"]["code"],C("opType")["create"]['code'],$gid,C("location")["panServer"]['code'],"","",$group['name_origin'],$this->mid,$this->user['login'],$this->user['uname'],$this->roleEnName,"",$conmment);
