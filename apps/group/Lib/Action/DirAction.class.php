...
class DirAction extends BaseAction {
	...
	function upload() {
		...
		if(isset($_POST['uploadsubmit']) || $_GET['ajax'] == 1) {
            ...
            if($info['status']){  //上传成功
				...
				$noteReplace = $sensitiveWord->checkSensitiveWord ( $attchement ['note'] );
				$noteReplace = json_decode ( $noteReplace, true );
				if ($noteReplace ["Code"] != 0) {
					return;
				}
				$attchement ['note'] = $noteReplace ['Data'];
            	...
            	if($result) {
                    ...
                    $conmment = $this->user['uname']."于".date("Y-m-d H:i:s",time())."在".C("appId")["xqqz"]['name']."上传了“".$info['info']["0"]["name"]."”的文件";
                    $logObj = $this->getLogObj(C("appType")["hdjl"]["code"],C("appId")["xqqz"]["code"],C("opType")["upload"]['code'],$info['info']["0"]["attach_id"],C("location")["attachServer"]['code'],"","",$info['info']["0"]["name"],$this->mid,$this->user['login'],$this->user['uname'],$this->roleEnName,"",$conmment);
	...
	function download()	{
		...
		if (3 == intval($this->groupinfo['whoDownloadFile']) && !$this->ismember) {
			$this->error('对不起，该圈子只允许圈子内成员下载文件');
           ...
           $file_info = $this->dir->where('attachId=\''.$_POST['fid'].'\' AND is_del=0')->find();
           $file = $this->apis_client->getFile($file_info["attachId"]);
            if(isset($file)){
			// 增加下载次数
	   		$this->dir->setInc('totaldowns', 'id=' . $_POST['fid']);
	   		// 积分
	...
	function download1() {
        ...
        if(!isset($_POST['fid'])) exit;
            $file_info = $this->dir->where('id=\''.$_POST['fid'].'\' AND is_del=0')->find();
            $file = $this->apis_client->getFile($file_info["attachId"]);
            $file_path = UPLOAD_PATH . '/' . $file_info['fileurl'];

            if(isset($file)){              //下载文档里附件的时候
			// 增加下载次数
	   		$this->dir->setInc('totaldowns', 'attachId=' . $_POST['fid']);
	   		// 积分
			X('Credit')->setUserCredit($this->mid, 'group_download_file');

			include_once(SITE_PATH . '/addons/libs/Http.class.php');
                redirect($file->url);
		}else if (file_exists($file_path)) {    //下载话题里附件的时候
			// 增加下载次数
	   		$this->dir->setInc('totaldowns', 'id=' . $_POST['fid']);
	   		// 积分
			X('Credit')->setUserCredit($this->mid, 'group_download_file');
			...
			if(strlen($file_info['name'])>31 && strlen($file_info['name'])<41)
			      $file_info['name'] = $file_info['name'].'.'.$file_info['filetype'];
			if(strlen($file_info['name'])>40){
				$fileNewName=substr_replace($file_info['name'],'...',41);
				 $file_info['name'] = $fileNewName.'.'.$file_info['filetype'];
			}
