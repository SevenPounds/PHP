    ...
    class BaseAction extends Action {
        ...
        protected function _initialize() {
          ...
          $groupinfo = D('Group')->where('id='.$this->gid." AND is_del=0")->find();
            if (!$groupinfo) {
              ...
              $this->error('该圈子不存在，或者被删除');
            }
            /* + ------ 后台全局限制 START */
		     $groupinfo ['openWeibo'] = 0;
	         $groupinfo ['openBlog'] = model ( 'Xdata' )->get ( 'group:discussion' ) ? $groupinfo ['openBlog'] : 1;
	         $groupinfo ['openUploadFile'] = model ( 'Xdata' )->get ( 'group:uploadFile' ) ? $groupinfo ['openUploadFile'] : 1;
		/* + ------ 后台全局限制 END */
	   ...
	   protected function _upload($path,$save_name,$is_replace,$is_thumb,$thumb_name,$thumb_max_width) {
        ...
        protected function _getSearchKey($key_name = 'k',$prefix="group_search") {
            ...
            $key_name_r = t($_REQUEST[$key_name]);
            if ( isset($key_name_r) && !empty($key_name_r) ) {
                $key_name_get = t($_GET[$key_name]);
                $key_name_post = t($_POST[$key_name]);
                if($key_name_get){
                    $key = html_entity_decode( urldecode($key_name_get) ,ENT_QUOTES);
                }elseif($key_name_post){
                    $key = $key_name_post;
                }
                // 关键字不能超过30个字符
