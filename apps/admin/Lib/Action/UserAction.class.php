...
class UserAction extends AdministratorAction {
	...
	public function category() {
		$pids = model('CategoryTree')->setTable('user_category')->getPids();
		$treeData = array();
		foreach ($pids as $v) {
			$_GET['pid'] = intval($v['pid']);
			$tmp = model('CategoryTree')->setTable('user_category')->getNetworkList($_GET['pid']);
			$treeData = array_merge($treeData, $tmp);
		}
		// 配置删除关联信息
		$this->displayTree($treeData, 'user_category', 2, '', '', 8);
	}
