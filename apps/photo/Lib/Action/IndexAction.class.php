...
class IndexAction extends BaseAction {
	...
	public function albums () {
		...
		for ($i = 0; $i < count($data['data']); $i++) {
			$photo_count += intval($data['data'][$i]['photoCount']);
			//第三方访问者取消隐私标题
			if($data['data'][$i]['userId']==$this->mid){
				//隐私标题
				switch ($data['data'][$i]['privacy']){
					case 4:$data['data'][$i]['photoPrivacy']="密码可见" ;break;
					case 3:$data['data'][$i]['photoPrivacy']="自己可见" ;break;
					case 2:$data['data'][$i]['photoPrivacy']="关注可见" ;break;
					break;
				}
			};
		}
		...
		$this->assign('photoCount', $photo_count);
		$this->assign('lastUpdateTime', $lastUpdateTime);
	...
	public function classalbum () {
		...
		$this->setTitle($this->appName.'：'.$album['name']);
	...
	public function photo() {
        ...
        $this->operationLog["remark"]="个人空间相片详情查看";
		...
		$this->setTitle($user_name.'的图片：'.$photo['name']);
	...
	public function classphoto() {
		...
		// 验证图片信息是否正确
		if (!$photo) {
			$this->assign('jumpUrl', U('photo/Index/classalbum', array('uid'=>$this->uid,'id'=>$aid,'cid'=>$classId)));
			$this->error('图片不存在或已被删除！');
		}
	...
	public function weiboalbum () {
		...
		$this->assign('photos', $photos);
		$this->setTitle(getUserName($this->uid).'的说说相册');
	...
	public function weibophoto () {
		...
		//截取图片名称
		$contentArr =split('[SITE_URL]',$feedInfo['content']);
		$feedInfo['content'] =$contentArr['0'];
		$feedInfo['content'] =substr($feedInfo['content'],0,$feedInfo['content'].length-4);
