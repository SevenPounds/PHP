...
class PhotoModel extends Model {
	 ...
	 */
	public function getDataWidthClassId($type, $loadId, $limit = 20,$classId){
		if ($type === 'hot') {
			$map['b.classId'] = $classId;
			$map['a.is_del'] = 0;
			$map['a.status'] = 1;
			$map['b.privacy'] = 1;
			...
			$order = 'a.id DESC';
			// 获取分页数据
			$result = D()->table('`'.$this->tablePrefix.'photo` AS a LEFT JOIN `'.$this->tablePrefix.'photo_album` AS b ON a.albumId = b.id')->field('a.*')->where($map)->order($order)->findPage($limit);
	...
	private function _formatImageSize ($data, $width = 225) {
		...
		foreach($data as &$value) {
			$imageInfo = getImageInfo($value['savepath']);
			$value['height'] = ceil($width * $imageInfo[1] / $imageInfo[0]);
			$value['width'] = $width;
		}
	 ...
	 * @param string $attach_ids 附件id
	 * @param integer uid 发布者uid
	 */
	public function syncToFeed($app,$content,$source_url,$attach_ids,$uid,$cid) {
		$d['content'] = $content;
		$d['body'] = $content;
		$d['source_url'] = $source_url;
		$d['attach_id'] = $attach_ids;
		!empty($cid) && $d['class_id'] = $cid;
		$feed = model('Feed')->put($uid, $app, 'postimage', $d, '', 'feed');
		return $feed['feed_id'];
	}
}