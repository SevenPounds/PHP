...
class AlbumModel extends Model {
	...
	/**
	 * 为班级创建默认数据
	 * @param integer $cid 校园班级id
	 * @return void
	 */
	public function createNewClassData ($cid) {
		// 创建默认相册
		if (intval($cid) <= 0) {
			return -1;
		}
		$count = $this->where("classId='$cid' AND isDel=0")->count();
		if ($count == 0) {
			$name = "默认班级相册";
			$album['cTime'] = time();
			$album['mTime'] = time();
	 ...
	 * @return boolean 是否删除成功
	 */
	public function deleteClassAlbum ($aids, $classid, $isAdmin = 0) {
		// 解析ID成数组
		$aids = is_array($aids) ? $aids : explode(',', $aids);
		// 非管理员只能删除自己的图片
		!$isAdmin && $map['classId'] = $classid;
		// 同步删除图片及附件
		$album['albumId'] = array('IN', $aids);
		$photos = D('Photo', 'photo')->field('id')->where($album)->findAll();
		foreach ($photos as $v) {
	...
	/**
	 * 根据用户id和相册id检查用户是否拥有该相册
	 * @param unknown_type $uid
	 * @param unknown_type $albumId
	 */
	public function checkOwner($uid,$albumId){
	 ...
	 * @return boolean
	 */
	public function isPrivate($albumId){
		$album = $this->find($albumId);
		if($album['privacy'] == 4 && strlen($album['privacy_data']) > 0){
			return true;
		}else{
			return false;
		}
