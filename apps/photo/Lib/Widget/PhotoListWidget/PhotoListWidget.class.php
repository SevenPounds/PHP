...
class PhotoListWidget extends Widget {
	...
	public function render ($data) {
		...
		if($data['cid']&& ($data['albumtype']=='class'||$data['albumtype']=='school')){
			$var['classId'] = intval($data['cid']);
		}
    ...
    public function loadMore () {
    	...
    	$classId = intval($_REQUEST['classId']);
    		...
    		// 获取HTML数据
    	$content = $this->getData($type, $loadLimit, $loadId,$classId);
     ...
     * @return array 照片流数据
     */
    public function getData ($type, $loadLimit, $loadId,$classId) {
		// 获取照片数据
	 	if($classId){
			$list = D('Photo', 'photo')->getDataWidthClassId($type, $loadId, $loadLimit,$classId);
			$var['classId'] = $classId;
	 	}else{
