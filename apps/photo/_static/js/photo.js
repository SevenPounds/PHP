...
photo.loadMore = function() {
	...
	$.get(U('widget/PhotoList/loadMore'), postArgs, function(res) {
		if(res.status == 1) {
			...
			// 分页操作
			if(photo.setting.loadcount >= photo.setting.loadmax) {
 ...
 * @return void
 */
photo.editphotoTab = function (albumId, photoId, nextId) {
	ui.box.load(U('photo/Manage/edit_photo_tab') + '&aid=' + albumId + '&pid=' + photoId + '&nextid=' + nextId, '编辑图片');
};
/**
 * 编辑班级图片弹窗
 * @param integer albumId 相册ID
 ...
 * @return void
 */
photo.editClassPhotoTab = function (albumId, photoId, classId, nextId) {
	ui.box.load(U('photo/Manage/edit_photo_tab') + '&aid=' + albumId + '&pid=' + photoId + '&cid=' + classId+ '&nextid=' + nextId, '编辑图片');
};
/**
 * 删除单张图片
 * @param integer albumId 相册ID
