...
function create_classalbum_tab (cid, isRefresh) {
	isRefresh = (typeof isRefresh === 'undefined') ? 0 : isRefresh;
	ui.box.load(U('photo/Manage/create_classalbum_tab') + '&cid=' + cid + '&isRefresh=' + isRefresh, '创建' + APP_NAME);
};
/**
 * 执行创建班级专辑操作
...
function do_create_classalbum (isRefresh,cid) {
	...
	$.post(U('photo/Manage/do_create_classalbum'), {name:name,cid:cid}, function(res) {
		if (res.status == -1) {
			ui.error('该相册名已存在');
