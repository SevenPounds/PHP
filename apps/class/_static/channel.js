...
channel.init = function(option)
{
	...
	this.setting.loadmax = option.loadmax || 1;					// 加载最大次数
	this.setting.loadId = option.loadId || 0;					// 加载起始ID
	this.setting.loadlimit = option.loadlimit || 20;			// 每次加载的数目，默认为10
	...
	this.setting.townname = option.townname || '';
...
channel.loadMore = function()
{
	...
	postArgs.cityname = channel.setting.cityname;
	postArgs.townname = channel.setting.townname;
	...
	$.get(U('widget/Content/loadMore'), postArgs, function(res) {
		...
		if(res.status == 1) {
			...
			// 动态加载数据
			channel.setting.page = res.page;	
			
			channel.dynamicLoading(res.html, false);

			// 分页操作
			if(channel.setting.loadcount >= channel.setting.loadmax) {
				$(channel.setting.container).after('<div id="page" class="page" style="display:none;padding-right:10px;">'+res.pageHtml+'</div>');
					...
					$('#page').find('a').each(function() {
						...
						if(href) {
							...
							$(this).click(function() {
								...
								$('#main-wrap').append('<div id="container"></div>');
			...
			$('#loadMore').remove();
			channel.dynamicLoading('<div class="pl12 left box" style="text-align:center;padding-left:40%;">未查询到相关数据</div>', false);
		}
	}, 'json');
	return false;
};
