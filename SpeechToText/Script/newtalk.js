...
function loadNewSTT(IntervalTime, containId) {
...
* @param {Number} 音频能量值(0-100)
*/
function newsetEnergy(energy) {
    if (energy < 0)
        energy = 0;
    if (energy > 100)
        energy = 100;
	var currentEnergy = energy * 0.25;
	if(energy < 25){
		currentEnergy = currentEnergy*5;
	}
    //setTimeout(function () {
   jQuery(".volumelist").children().each(function(i){
	   if(i == 9){
		   $(this).css("height",(currentEnergy*0.2+5)+"px")
	   }
	   if(i == 10){
		   $(this).css("height",(currentEnergy*0.4+5)+"px")
	   }
	    if(i == 11){
		   $(this).css("height",(currentEnergy*0.6+5)+"px")
	   }
	    if(i == 12){
		   $(this).css("height",(currentEnergy+5)+"px")
	   }
	    if(i == 13){
		   $(this).css("height",(currentEnergy*0.6+5)+"px")
	   }
...
AudioPlayer.newstop = function (key, obj) {
	_recorder.stopAudio();
	$(obj).html('<em class="jing"></em>');
	$(obj).attr("onclick", "AudioPlayer.audioplaynew('"+key+"', this)");
    //alert("停止播放");
}
...
//播放按钮
AudioPlayer.audioplaynew = function(key, obj){
	_recorder.stopAudio();
	AudioPlayer.newplayEnd();
	 var url = SpeechRecorderSettings.RecordServer + key;
    _recorder.loadAudio(url, true)
	$(obj).html('<em class="dong"></em>');
	$(obj).attr("onclick", "AudioPlayer.newstop('"+key+"', this)");
}

//结束播放
AudioPlayer.newplayEnd = function(){
	_recorder.stopAudio();
	$("a#newplaybtn").each(function(){
		$(this).html('<em class="jing"></em>');
		$(this).attr('onclick', "AudioPlayer.audioplaynew('"+$(this).attr('name')+"', this)");
	});
	if($("#playbtn").attr("class").indexOf("disable") == -1){
		$("#playbtn").attr("class","inline-block icon-play");
	}
}
/*
* 设置进度条
