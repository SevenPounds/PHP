...
class Log {
     ...
     * @return void
     */
    static function writeLog($message,$type=self::FILE,$destination='') {
        ...
        if(self::FILE == $type) { // 文件方式记录日志
            //检测日志文件大小，超过配置大小则备份日志文件重新生成
            //if(is_file($destination) && floor(C('LOG_FILE_SIZE')) <= filesize($destination) )
            if(is_file($destination) && floor(C("LOGRECORD_SIZE")) <= filesize($destination) )
                rename($destination,dirname($destination).'/'.time().'-'.basename($destination));
        }
        $head = ["appName"=>"EduSNS","nodeIP"=>get_client_ip(),"logTime"=>date("Y-m-d H:i:s",time()),"traceId"=>""];
        $body = json_decode($message);
        $message = json_encode(["head"=>$head,"body"=>$body],JSON_UNESCAPED_UNICODE);
        error_log("{$message}\r\n",$type,$destination);
        //clearstatcache();
    }
}