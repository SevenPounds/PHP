...
function throw_exception($msg,$type='') {
    ...
    if(class_exists($type,false))
    	...
    	//update by zhehuang 更新ts的错误链接跳转
    	redirect(U('public/Index/error_page',array('msg'=>$msg)));
...
function fetch($templateFile='',$tvar=array(),$charset='utf-8',$contentType='text/html',$display=false) {
    ...
    // 模版不存在
     ...
     unset($tvar['templateCacheFile']); //修复漏洞
    //模版缓存文件
...
function parse_html($html){
    ...
    $html = str_replace('[PREVIEW_SITE_URL]',PREVIEW_SITE_URL,$html);
    //外网链接地址处理
...
function parseForApi($html){
    ...
    $html = str_replace(array('[PREVIEW_SITE_URL]','&nbsp;'),array(PREVIEW_SITE_URL,' '),$html);
    //@提到某人处理
...
function replaceUrl($content){
    ...
    $content = str_replace('[PREVIEW_SITE_URL]', PREVIEW_SITE_URL, $content);
...
function replaceMessage($content){
    ...
    $content = str_replace('[PREVIEW_SITE_URL]', PREVIEW_SITE_URL, $content);
    $content = str_replace('&nbsp;', "", $content);
    $content = preg_replace_callback('/((?:https?|mailto|ftp):\/\/([^\x{2e80}-\x{9fff}\s<\'\"“”‘’，。}]*)?)/u', '_parse_message', $content);
    //$index =  strripos($content,"点击查看");
    return $content;
}
...
function getThumbImage($filename,$width=100,$height='auto',$cut=false,$replace=false){
    ...
    $oldFile   = $info['dirname'].DIRECTORY_SEPARATOR.$info['filename'].'.'.strtolower($info['extension']);
    $thumbFile = $info['dirname'].DIRECTORY_SEPARATOR.$info['filename'].'_'.$width.'_'.$height.'.'.strtolower($info['extension']);
    ...
    //执行缩图操作
        ...
        if($old_image_width<=$width && $old_image_height<=$height){
            ...
            //生成缩略图
            if(strtolower(substr($filename, strrpos($filename, '.')+1))=='png'){
            	$cut = false;
            }
            if($cut){
...
function getFollowState($uid,$fid,$type=0) {
    ...
    if ($uid==$fid)
        return 'unfollow';
    if (M('user_follow')->where("(uid=$uid AND fid=$fid) OR (uid=$fid AND fid=$uid) AND type=$type")->count() == 2) {
        return 'eachfollow';
    }else if ( M('user_follow')->where("uid=$uid AND fid=$fid AND type=$type")->count()) {
        return 'havefollow';
    }else {
        return 'unfollow';
    }
 ...
 * @param int $cid
 * @param int $type
 * @param string  $sizeKey  头像大小
 */
function getCampuseAvatar($cid,$type,$sizeKey){
	$empty_url = THEME_URL.'/_static/image/noavatar';
	$avatar_big = $type==1?'img_school_big.jpg':'img_class_big.jpg';
	$avatar_middle = $type==1?'img_school_middle.jpg':'img_class_middle.jpg';
	$avatar_small = $type==1?'img_school_small.jpg':'img_class_small.jpg';
	$avatar_tiny = $type==1?'img_school_tiny.jpg':'img_class_tiny.jpg';

	$avatar_url = array(
			'avatar_original' 	=> $empty_url.'/'.$avatar_big,
			'avatar_big' 		=> $empty_url.'/'.$avatar_big,
			'avatar_middle' 	=> $empty_url.'/'.$avatar_middle,
			'avatar_small' 		=> $empty_url.'/'.$avatar_small,
			'avatar_tiny' 		=> $empty_url.'/'.$avatar_tiny,
	);
	if($type==1){
		$avatar_path='/avatar/school';
	}elseif($type==2){
		$avatar_path='/avatar/class';
	}
	$original_file_name = $avatar_path.convertCidToPath($cid,$type).'/original.jpg';
...
/**
 * 将用户的fID转换为三级路径
 * @param integer $cid 用户cid
 * @param integer $type 类型
 * @return string 用户路径
 */
function convertCidToPath($cid,$type) {
	if($type==1){
		$avatar_path='avatar_campuspath_';
	}elseif($type==2){
		$avatar_path='avatar_classpath_';
	}
	// 静态缓存
	$sc = static_cache($avatar_path.$cid);
	if(!empty($sc)) {
		return $sc;
	}
	$md5 = md5($cid);
	$sc = '/'.substr($md5, 0, 2).'/'.substr($md5, 2, 2).'/'.substr($md5, 4, 2);
	static_cache($avatar_path.$cid, $sc);
	return $sc;
}

/**
 * 获取表扬图标
 * @param string $icon 数据库icon图片
 * @return string
 */
function getPraiseIconUrl($icon){
	return THEME_PUBLIC_URL.'/image/praise/'.$icon;
}
 ...
 */
function get_classphoto_url ($savepath, $width = 200, $height = 200, $cut = true) {
 ...
 */
function get_classalbum_cover ($albumId, $album = '', $width = 140, $height = 140) {
	// 获取相册详细信息
	if (empty($album) || $albumId != $album['id']) {
		$album = D('Album', 'class')->find($albumId);
	}
	// 根据隐私情况，判断相册封面
	if ($album['privacy'] == 4 && (md5($album['privacy_data'].'_'.$album['id'].'_'.$album['userId']) != $_COOKIE['album_password_'.$album['id']])) {
		...
		if (intval($album['photoCount']) > 0 && !empty($album['coverImagePath'])) {
			...
			$firstImg = D('Photo', 'class')->field('savepath')->where("albumId={$album['id']}")->order('`order` DESC,id DESC')->find();
 ...
 */
function getHomeUrl($orgnization){
	if($orgnization['type']==1)
		return U('class/CampusHome/schoolIndex',array('cid'=>$orgnization['id']));
	else
		return U('class/ClassHome/index',array('cid'=>$orgnization['id']));
}
 ...
 * @param array $orgnization
 */
function getInfoBoxHtml($orgnization){

	$follow = D('ClassData','class')->getClassDataByFid($orgnization['id'],'follower_count',$orgnization['type']);
	$followercount = $follow['value']?$follow['value']:0;
	switch($orgnization['type']){
		case 1:
			$studentcount =  D('CyStatistics')->count_student_by_school($orgnization['id']);
			$teachercount =  D('CyStatistics')->count_teacher_by_school($orgnization['id']);
			$classcount =   D("CyStatistics")->count_class_by_school($orgnization['id']);
			$spaceUrl  =U('class/CampusHome/schoolIndex',array('cid'=>$orgnization['id']));
			$followerUrl = U('class/CampusHome/campus_follower',array('cid'=>$orgnization['id']));
			$boxhtml = '<div class="school_inforbox">
			<div class="infor_boxbtn"><p class="green pt4"><a href="'.$spaceUrl.'">'.$classcount.'</a></p><p class="pt2 black"><a href="'.$spaceUrl.'">班级</a></p></div>
			<div class="infor_boxbtn"><p class="green pt4"><a href="'.$spaceUrl.'">'.$teachercount.'</a></p><p class="pt2 black"><a href="'.$spaceUrl.'">教师</a></p></div>
			<div class="infor_boxbtn"><p class="green pt4"><a href="'.$spaceUrl.'">'.$studentcount.'</a></p><p class="pt2 black"><a href="'.$spaceUrl.'">学生</a></p></div>
			<div class="infor_boxbtn"><p class="green pt4"><a href="'.$followerUrl.'"><span class="follower_'.$orgnization['id'].'">'.$followercount.'</span></a></p><p class="pt2 black"><a href="'.$followerUrl.'">粉丝</a></p></div>
			<div class="clear"></div></div>';
			break;
		case 2:
			$studentcount =  D('CyStatistics')->count_student_by_class($orgnization['id']);
			$teachercount =  D('CyStatistics')->count_teacher_by_class($orgnization['id']);
			$parentscount =    D('CyStatistics')->count_parents_by_class($orgnization['id']);
			$spaceUrl  = U('class/ClassHome/index',array('cid'=>$orgnization['id']));
			$peopleUrl = U('class/ClassHome/class_people',array('cid'=>$orgnization['id']));
			$followerUrl = U('class/ClassHome/class_follower',array('cid'=>$orgnization['id']));
			$boxhtml = '
			<ul>
			<li><a href="'.$peopleUrl.'"><p class="green pt4">'.$parentscount.'</p><p class="pt2 black">家长</p></a></li>
			<li><a href="'.$peopleUrl.'"><p class="green pt4">'.$teachercount.'</p><p class="pt2 black">教师</p></a></li>
			<li><a href="'.$peopleUrl.'"><p class="green pt4">'.$studentcount.'</p><p class="pt2 black">学生</p></a></li>
			<li><a href="'.$followerUrl.'"><p class="green pt4"><span class="follower_'.$orgnization['id'].'">'.$followercount.'</span></p><p class="pt2 black">粉丝</p></a></li>
			<div class="clear"></div>
			</ul>';
			break;
		default:
...
function getClassInfoBox($orgnization){
	...
	$followerUrl = U('class/ClassHome/class_follower',array('cid'=>$orgnization['id']));
	$peopleUrl = U('class/ClassHome/class_people',array('cid'=>$orgnization['id']));
	$boxhtml = '<p class="pt8">老师<font class="green"><a href="'.$peopleUrl.'">
	'.$teachercount.'</a></font> | 学生<font class="green"><a href="'.$peopleUrl.'">
	'.$studentcount.'</a></font> | 家长<font class="green"><a href="'.$peopleUrl.'">
	'.$parentscount.'</a></font> | 粉丝<font class="green"><a href="'.$followerUrl.'">
	'.$followercount.'</a></font></p>';
	return $boxhtml;
}
 ...
 * @param array $orgnization
 */
function getOrgnizationArea($orgnization,$length){
	$area =  Model('CyArea')->getFullAreaByOrgnization($orgnization);
	if($length == 2){
		return $area['city']['name'].' '.$area['district']['name'];
	}
 ...
 *
 */
function getCountByType($type,$orgnization){
	switch($type){
		case 'follower':
			$follow = D('ClassData','class')->getClassDataByFid($orgnization['id'],'follower_count',$orgnization['type']);
			$count = $follow['value'];
 ...
 */ 
function getOrgnizationIntro($orgId,$type=1){
	$orgnization = D('OrgInfo','class')->where("fid ='{$orgId}' AND type ='{$type}'")->find();
	if($orgnization){
		return $orgnization['intro'];
	}
	return "";
}
/**
 * 记录个人来访_visitor
 * @param $uid 被访问用户的UID
 * @param $vuid 访问用户的ID，即系统当前登录用户
 * @param $timeLimit 最短记录时间间隔，默认为1小时，单位为秒
 * @author yxxing
 */
function recordVisitor($uid, $vuid, $timeLimit = 3600){
	if(!$uid || !$vuid || $uid == $vuid){
		return FALSE;
	}
	//SESSION是否记录了当前用户的访问信息
	if(isset($_SESSION['visitor_records'])){
		$visitRecords =  $_SESSION['visitor_records'];
	} else{
		$visitRecords = array();
	}
	//当前时间
	...
	//是否更新数据库，添加来访次数
	$toUpdate = false;
	if(array_key_exists($uid, $visitRecords)){
		//SESSION中记录了对该用户访问的信息
		$lastVisitTime = $visitRecords[$uid];
		// 如果SESSION记录时间超过一小时，则添加访问次数
		($nowTime > ($lastVisitTime + $timeLimit)) &&  $toUpdate = true;
	} else {
		//SESSION中没有该记录，则查询数据库，记录时间是否在1小时内
		$vtime = D('UserVisitor')->where('uid='.$uid.' AND vuid='.$vuid)->getField('vtime');
		($nowTime > $vtime + $timeLimit) &&  $toUpdate = true;
		//将数据库中记录的用户访问时间添加到session
		$visitRecords[$uid] = $nowTime;
		$_SESSION['visitor_records'] = $visitRecords;
	}
	model('UserVisitor')->updateVisitor($uid, $vuid);
	//查询今天来访
	...
	if($perday_result){
		...
		//今天开始时间
		$todayStartTime = strtotime($nowTime);
		//今天结束时间
		$todayEndTime = $todayStartTime + 60*60*24;
		//来访记录时间在当天内则增加
		if($last_time > $todayStartTime && $last_time < $todayEndTime){
			$toUpdate && D('UserData') ->updateKey("perday_visitor_count", 1, true, $uid);
			//来访记录时间不在当天内则归零+1
		} else{
			$now_update_time = date("Y-m-d H:i:s",time());
			$map = array("key" =>"perday_visitor_count", "uid"=>$uid);
			D('UserData') ->where($map)->save(array("value"=>"1", "mtime"=>$now_update_time));
		}
	} else{
		D('UserData') ->updateKey("perday_visitor_count", 1, true, $uid);
	}
	//添加来访次数
	if($toUpdate){
		//更新SESSION
		$visitRecords[$uid] = time();
		$_SESSION['visitor_records'] = $visitRecords;
			
		D('UserData') ->updateKey("visitor_count", 1, true, $uid);
	}
    return TRUE;
}
/**
 ...
 * @return string
 */
function yunUrlEncode($str) {
	return str_replace('%', '-', urlencode($str));
}
/**
 * 解决上传文件中文名问题，配合yunUrlEncode使用
 * @param string $str
 * @return string
 */
function yunUrlDecode($str) {
 ...
 * 获取资源在资源网站预览地址
 * @param string $rid
 * @param string or array $app
 */
function Ures($url,$params = false){
	//普通模式
	if(false==strpos($url,'/')){
		$url    .='//';
	}
	...
	//填充默认参数
	$urls   =   explode('/',$url);
	$app    =   isset($urls[0]) && !empty($urls[0]) ? $urls[0] : C('RESLIB_VIEW_APP');
	$mod    =   isset($urls[1]) && !empty($urls[1]) ? ucfirst($urls[1]) : 'Rescenter';
	$act    =   isset($urls[2]) && !empty($urls[2]) ? $urls[2] : 'detail';
	
	//组合默认路径
	$site_url   =   C('RS_SITE_URL').'/index.php?app='.$app.'&mod='.$mod.'&act='.$act;
