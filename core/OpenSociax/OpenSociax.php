...
if(!empty($_REQUEST['templateCacheFile'])){

	header("location: addons/theme/stv1/errorPage/error.html");
...
if (isset($_POST['PHPSESSID'])) {
    ...
    session_start();//避免通过User-Agent: Shockwave Flash发出请求没有SESSION信息问题
}
...
tsconfig(include CONF_PATH.'/env.inc.php');
tsconfig(include CONF_PATH.'/access.inc.php');
tsconfig(include CONF_PATH.'/router.inc.php');
tsconfig(include CONF_PATH.'/log.config.php');
tsconfig(include CONF_PATH.'/module_name.inc.php');
...
//定义资源预览地址
tsdefine('PREVIEW_SITE_URL'	, Ures('//'));
