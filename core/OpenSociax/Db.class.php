...
class Db extends Think
{
    ...
    protected function initConnect($master=true) {
        if(1 == C('DB_DEPLOY_TYPE'))
            ...
            // 默认单数据库
            if ( !$this->connected ) $this->linkID = $this->connect();
    }
    ...
    public function connect($config='',$linkNum=0,$force=false) {
        if ( !isset($this->_linkID[$linkNum]) ) {
            ...
            if($pconnect) {
                $this->_linkID[$linkNum] = mysql_pconnect( $host, $config['username'], $config['password'],131072);
            }else{
                $this->_linkID[$linkNum] = mysql_connect( $host, $config['username'], $config['password'],true,131072);
            }
            if ( !$this->_linkID[$linkNum] || (!empty($config['database']) && !mysql_select_db($config['database'], $this->_linkID[$linkNum])) ) {
                //E(mysql_error());
            }
            $dbVersion = mysql_get_server_info($this->_linkID[$linkNum]);
            //使用UTF8存取数据库
            mysql_query("SET NAMES '".$config['charset']."'", $this->_linkID[$linkNum]);
            //设置 sql_model
            if($dbVersion >'5.0.1'){
                mysql_query("SET sql_mode=''",$this->_linkID[$linkNum]);
            }
            // 标记连接成功
            $this->connected    =   true;
            // 注销数据库连接配置信息
