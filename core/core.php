...
tsdefine('LOG_PATH'			,	SITE_PATH.'/logs/');
...
function tsload($filename) {
	...
	if (!isset($_importFiles[$key])) {
		if (is_file($filename) && strpos ( $filename ,  SITE_PATH ) !== false){
 ...
 * 打印PHP调用堆栈信息
 * 示例：
 * file: E:\Workspace10\EduSNS4.2\core\OpenSociax\Db.class.php Db=>debug [line 120]
 * 备注：call_user_func方式函数调用，file,line都是空
 * usage: 调试、打印错误调用堆栈，不推荐频繁调用，频繁写日志影响性能
 * @author yuliu2
 */
function print_function_stack(){
	$stacks = debug_backtrace();
	$trace = "";
	foreach($stacks as $stack){
		$trace .= "\r\n file: " . $stack['file'] . " " .  $stack['class'] . "=>" .  $stack['function'] .  " [line " . $stack['line'] . "]";
	}
	Log::write($trace, Log::DEBUG);
}
?>
