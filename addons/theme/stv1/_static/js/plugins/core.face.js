...
core.face = {
		...
		createDiv:function(){
			...
			var height = $(this.faceObj).height();
			if($(this.faceObj).attr("name") == "wbhome"){
				//图标展示位置修改后重新设置定位
				var height_offset =  (height > 20) ? height / 2 + 5 : height / 2;
				$('#emotions').css({top:(pos.top + height_offset) + "px", left:(pos.left -352- 5) + "px", "z-index":1001});
				$(".arrow-t").css("left","397px");
			}
			else{
				var height_offset =  (height > 20) ? height / 2 + 5 : height / 2;
				$('#emotions').css({top:(pos.top + height_offset) + "px", left:(pos.left - 5) + "px", "z-index":1001});
			}
