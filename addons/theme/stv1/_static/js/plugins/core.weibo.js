...
core.weibo = {
	...
	init:function(agrs) {
		...
		this.gid = args.gid;
		this.fromChannel=args.fromChannel;  //是否展示知识堂微博sjzhao
...
//		}
	...
	loadMoreFeed: function() {
		...
		// 获取微博数据
		if(_this.fromChannel){
			var geturl=U('widget/ChannelFeedList/loadMore');
			var getdata={'loadId':_this.loadId, 'type':_this.feedType, 'uid':_this.uid, 'feed_type':_this.feed_type, 'feed_key':_this.feed_key, 'fgid':fgid, 'topic_id':_this.topic_id, 'load_count':_this.loadCount, 'gid':_this.gid, 'widget_appname':'channel'};
		}else{
			var geturl=U('widget/FeedList/loadMore');
		    var getdata={'loadId':_this.loadId, 'type':_this.feedType, 'uid':_this.uid, 'feed_type':_this.feed_type, 'feed_key':_this.feed_key, 'fgid':fgid, 'topic_id':_this.topic_id, 'load_count':_this.loadCount, 'gid':_this.gid};
		}
		$.get(geturl, getdata, function(msg) {
			// 加载失败
...
//					_this.startNewLoop();
				...
				if(_this.loadCount >= 4) {
					...
					msg.html = $lastDl.find('div').filter('.cy_mr_box').slice(30);
				}
	...
	post_feed: function(_this, mini_editor, textarea, isbox , url) {	
		...
		if(attachobj.length > 0) {
			var type; 
			if(attachobj.attr('feedtype') == 'share'){
				type = 'share';
			}else{
				type = (attachobj.attr('feedtype') == 'image') ? 'postimage' : 'postfile';
			}
		...
		//alert(url);exit;
		// 发布微博
		$.post(url, {body:data, type:type, app_name:app_name, content:'', record_id:record_id,attach_id:attach_id,videourl:videourl, channel_id:channel_id, source_url:attrs.source_url, blog_id:attrs.blog_id, gid:attrs.gid, feed_title:attrs.feedTitle, class_id:attrs.class_id}, function(msg) {
			obj.isposting = false;
			...
			if(msg.status == 1) {
				...
				if(record_id == ""){
					...
					$("#talkpostok").fadeIn('fast',function(){
						$("#talkpostok").fadeOut(1000);
					});
					//清除内容
					$(".sayvolume").hide();	
					$("#playbtn").attr("class", "inline-block icon-play disable");
					$(".saycontent").html("说点什么吧...");
				}
