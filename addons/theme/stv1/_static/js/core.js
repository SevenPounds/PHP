...
/**
 * 资源预览地址
 * 模拟TS的U函数，需要预先定义JS全局变量RS_SITE_URL 和 RS_VIEW_APP
 * @param string url 链接地址
 * @param array params 链接参数
 * @return string 组装后的链接地址
 */
var Ures = function(url,params) {
	var website = RS_SITE_URL+'/index.php';
	url = url.split('/');
	if(url[0]=='' || url[0]=='@')
		url[0] = RS_VIEW_APP;
	if (!url[1])
...
var ui = {
	...
	showMessage: function(message, error, lazytime) {
		...
		$('#ui_messageBox').css({
			...
			$('#ui_messageBox').prepend('<iframe style="z-index:-1;position: absolute;visibility:inherit;width:'+_w+'px;height:'+_h+'px;top:0;left:0;filter=\'progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)\'"'+
	...
	showblackout: function() {
		...
		if($('.boxy-modal-blackout').length > 0) {
    		...
    		height = height > 1100 ? height : 1100; //chengcheng3 add 2014/3/6 默认遮罩页面高度
    		var divHtml = '<div class ="boxy-modal-blackout" ><iframe style="z-index:-1;position: absolute;visibility:inherit;width:'+$('body').width()+'px;height:'+height+'px;top:0;left:0;filter=\'progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)\'"'+
   		 'src="about:blank"  border="0" frameborder="0"></iframe></div>';
    		try{
    			if($('#teacher_studio_iframe',window.parent.document).length > 0 ){
        			height = 0;
        			divHtml = '<div class="boxy-modal-blackout"><iframe style="z-index:-1;position: absolute;visibility:inherit;width:'+$('body').width()+'px;height:'+height+'px;top:0;left:0;filter=\'progid:DXImageTransform.Microsoft.Alpha(style=0,opacity=0)\'"'+
        	   		 'src="about:blank"  border="0" frameborder="0"></iframe></div>';
        		}
    		}catch(e){
	 ...
	 */
	box: {
		WRAPPER: '<div class="wrap-layer" id="tsbox" style="display:none;z-index: 1000;">\
     			  <div class="content-layer" style="position: relative;background: #fff;border: 1px solid #bbb;">\
     			  <div class="layer-content" id="layer-content" style="text-align: left;"></div>\
     			  </div></div>',
		inited: false,
		...
		init: function(title, callback) {
			...
			core.loadCss(url);
			// 添加头部 判断title为空的情况下不显示头部 by xypan 2014/4/18
			if("undefined" != typeof(title) && title) {
				$("<div class='hd' style='background: #f1f1f1;font-weight: 700;font-size: 14px;color: #535353;padding: 9px 15px;cursor: move;text-align: left;'>"+title+"<a class='ico-close' href='#' style='position: absolute;right: 10px;top: 10px;width: 18px;height: 18px;'></a></div>").insertBefore($('#tsbox .layer-content'));
			}
		...
		load:function(requestUrl,title,callback,requestData,type) {
			...
			jQuery.ajax({url:requestUrl,
				...
				success:function(html){
					...
					// 在弹出框成功弹出的时候增加回调函数的内容 by xypan 0922
					if(requestData != null && typeof(requestData.callback) == "function"){
