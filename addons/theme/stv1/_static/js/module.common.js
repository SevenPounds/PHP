...
M.addEventFns({
			...
			if(cid  && parseInt(cid)>0){
				...
				if("undefined" == typeof(core.facecard)){
					core.plugFunc('facecard',function(){
						core.facecard.showClass(obj,cid);
					})
				}else{
					core.facecard.showClass(obj,cid);
				}
	...
	up_down: {
		click:function(){
			...
			if(attrs.from=='pcenter'){//from标记来自个人中心的事件sjzhao
				if($(this).hasClass('sq')){
					$(this).removeClass('sq');
					$(this).addClass('zk');
					$('#'+attrs.id).hide();
				...
				if($(this).hasClass('arrow-nav-up')){
					$(this).removeClass('arrow-nav-up');
					$(this).addClass('arrow-nav-down');
					$('#'+attrs.id).hide();
					$(this).attr('title',L('PUBLIC_OPEN'));
				}else{
					$(this).removeClass('arrow-nav-down');
					$(this).addClass('arrow-nav-up');
	...
	share_to_feed:{//分享操作
		load: function () {
			...
			if (attrs.isLoad == 1) {
				...
				var blogId = attrs.blogId;
				var color = attrs.color;
				var class_id = attrs.classId;
				var feedTitle = attrs.feedTitle ? attrs.feedTitle : "";
				var url = U('public/Share/shareToFeed')+'&initHTML='+initHTML+'&attachId='+attachId+'&from='+from+'&appname='+appname+'&source_url='+source_url+'&blogId='+blogId+'&feedTitle='+feedTitle+'&color='+color+'&class_id='+class_id;
				ui.box.load(url,'分享');
			}
			return false;
		},
...
var follow = {
	...
	createBtn: function(node, btnType) {
		...
		if (args.type == "studio" || args.type == "ms_studio"){
			btnClass = this.studioBtnClass[btnType];
			btnHTML = btnText;
		}
		...
		if (args.type == "studio" || args.type == "ms_studio") {
			...
			switch(btnType) {
				...
				case "eachFollow":
					$(node).bind({
						mouseover: function() {
							this.className = 'see_cancle';
							this.innerHTML = follow.btnText.unFollow;
						},
						mouseout: function() {
	...
	doFollow: function(node) {
		...
		$.post(url, {fid:args.uid}, function(txt) {
			if(1 == txt.status ) {
				...
				//学校类型
				if("undefined" != typeof(args.ftype) && args.ftype){
					node.setAttribute("href", [ U('public/Follow/unFollow'), '&fid=', args.uid, '&ftype=',args.ftype].join(""));	
					$(".follower_"+args.uid).each(function(){$(this).text(parseInt($(this).text())+1)});
				}
				else{
					node.setAttribute("href", [ U('public/Follow/unFollow'), '&fid=', args.uid].join(""));	
				}
				M.setEventArgs(node, ["uid=", args.uid, "&uname=", args.uname, "&following=", txt.data.following, "&follower=", txt.data.follower,'&ftype=',args.ftype,'&type=',args.type].join(""));
		...
		$.post(url, {fid:args.uid}, function(txt) {
			...
			if ( 1 == txt.status ) {
				...
				if ( "following_list" == args.refer ) {
					...
					if("undefined" != typeof(args.ftype)&&args.ftype){
						$(".follower_"+args.uid).each(function(){$(this).text(parseInt($(this).text())-1)});
						node.setAttribute("href", [U('public/Follow/doFollow'), '&fid=', args.uid,'&ftype=',args.ftype].join(""));
					}else{
