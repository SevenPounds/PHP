...
class AttachModel extends Model {
    ...
    protected $fields =	array(0 => 'attach_id',1=>'app_name',2=>'table',3=>'row_id',
    							...
    							15=>'save_name',16=>'save_domain',17=>'from',18=>'cid',19=>'downloadtimes',20=>'uname',21=>'width',22=>'height'
    						);
	...
	public function upload($data = null, $input_options = null, $thumb = false) {
		...
		$default_options['module'] =	C('CLIENT_SNS_APP_ID');
		$default_options['action'] =	'SNS上传附件';
		// 定制化设这，覆盖默认设置
	...
	public function localUpload($options){
		// 初始化上传参数
	...
	public function saveInfo($upload_info,$options){
		$data = array(
			'table' => t($data['table']),
		...
		if($options['save_to_db']) {
			foreach($upload_info as $u) {
				...
				$data['cid']=$options['cid'];//保存班级id
				$data['uname']=$options['uname'];//保存用户名
				//$data['save_domain'] = C('ATTACH_SAVE_DOMAIN'); 	//如果做分布式存储，需要写方法来分配附件的服务器domain
				if (in_array($u['extension'], array('jpg', 'gif', 'png', 'jpeg', 'bmp')) && !in_array($options['attach_type'], array('feed_file', 'weiba_attach'))) {
	...
	/**
	 * 保存系统推荐的默认头像
	 * @param unknown $saveImg_info
	 * @param unknown $options
	 * @return multitype:number string unknown Ambigous <string, number> Ambigous <number, string>
	 */
	public function saveDefaultImg($saveImg_info,$options){
		$data = array(
				'table' => t($data['table']),
				'row_id' => t($data['row_id']),
				'app_name' => t($data['app_name']),
		...
		$data['name'] = $saveImg_info['name'];
		$data['type'] = $saveImg_info['type'];
		$data['size'] = $saveImg_info['size'];
		$data['extension'] = strtolower($saveImg_info['extension']);
		$data['hash'] = $saveImg_info['hash'];
		$data['save_path'] = $options['custom_path'];
		$data['save_name'] = $saveImg_info['save_name'];
		$data['cid']=$options['cid'];//保存班级id
		$data['uname']=$options['uname'];//保存用户名
		$data['width'] = $saveImg_info['width'];
		$data['height'] = $saveImg_info['height'];
		$aid = $this->add($data);
		if(!empty($aid)){
			return "success";
		}else{
	 ...
	 * @author sjzhao
	 */
	public function getAttachNumByCid($cid){
		$map['cid']=$cid;
		return $this->where($map)->count();
	}
	public function UpdatedownloadtimesById($attachid){
		$map['attach_id']=$attachid;
		return $this->where($map)->setInc('downloadtimes');
	}
}