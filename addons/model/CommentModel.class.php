...
class CommentModel extends Model {
	...
	protected $fields = array('comment_id','app','table','row_id','app_uid','uid','content','content_origin', 'to_comment_id','to_uid','data','ctime','is_del','client_type','is_audit','storey');
    ...
    public function getCommentList($map = null, $order = 'comment_id ASC', $limit = 10, $isReply = false) {
       ...
       // dump($data);exit;
        ...
        foreach($data['data'] as &$v) {
			...
			/** *************** update by zhehuang 2015-02-07 start **************** */
        	/**
        	 * 由于班级空间的作业与讲解的评论导致空间消息出现样式问题，通过修改这部分的消息格式，
        	 * 解决异常问题
        	 */
        	if($v['table'] == 'yun_exercise'){
        	...
        	if($v['table'] == 'yun_exercise'){
        		$v['sourceInfo']['source_body'] = $replyInfo['content'];
        		$v['sourceInfo']['source_user_info']['uid'] = $replyInfo['user_info']['uid'];
        	}
        	/** *************** update by zhehuang 2015-02-07 end **************** */
    ...
    public function addComment($data, $forApi = false, $notCount = false, $lessUids = null) {
        ...
        $add['content_origin'] = $data['content_origin'];
...
/*       	    if (isset($uid) && !in_array($uid, array($c_v['app_uid'], $c_v['uid']))) {
     ...
     * @return array 评论列表信息
     */
    public function getExcellentCommentList($map = null, $order = 'digg_count DESC', $limit = 10, $isReply = false){
    	!$map['app'] && $this->_app && ($map['app'] = $this->_app);
    	!$map['table'] && $this->_app_table && ($map['table'] = $this->_app_table);
    	!isset($map['is_del']) && ($map['is_del'] = 0);
    	$map['digg_count'] = array('gt',0);
