...
class FeedTopicModel extends Model {
	...
	// 获取话题详细信息
	public function getTopicCount($topic_name = null)
	{
		if ($topic_name) {
			$topic_name = $topic_name;
			$map['topic_id'] = $this->getTopicId($topic_name);
			$mapParam['feed_id'] = array('in',getSubByKey(D('feed_topic_link')->where('topic_id='.$map['topic_id'])->field('feed_id')->select(),'feed_id'));
			$mapParam['_string'] = ' (is_audit=1 OR is_audit=0 AND uid='.$GLOBALS['ts']['mid'].') AND is_del = 0 ';
			$topic = model('Feed')->getList($mapParam);
			$count = 0;
			if($topic) {
				if($topic['data']) {
					$count = sizeof($topic['data']);
				}
			}
			return $count;
		} else {
