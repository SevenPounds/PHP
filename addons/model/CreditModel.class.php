...
class CreditModel extends Model {
	...
	public function getUserRanking($uid){
		$record = D('credit_user')->where(array('uid'=>$uid))->find();
		$result = array();
		$creditUserDao = M('credit_user');
		if($record){
			$score = $record['score'];
			if($score == null) {
				$score = 0;
				$rank = $creditUserDao->where("score > {$score}")->count();
				$total = $creditUserDao->count();
				$result['total'] = $total;
				$result['rank'] = $rank;
				$result['exceed'] = 0;
			} else {
				...
				$result['total'] = $total;
				$result['rank'] = $rank;
				$exceed = ($score==0) ? 0:intval(($total - $rank) / $total * 100);
    ...
    /**
     * 根据登录名和积分动作，添加新的积分记录（名师工作室）
     * @param string $login 用户登录名
     * @param string $action 积分动作
     * @return array() {'statuscode':'200 | 400','message':'**'}
     */
    public function addCreditLog($login, $action){
       $user = D('User')->where(array('login'=>$login))->find();
        if(empty($user)){
            $result['statuscode'] = '400';
            $result['messageEn'] = 'No such User, failed !';
            $result['message'] = '系统无此用户，添加失败！';
            Log::write($login . "--" . $action . json_encode($result), Log::INFO);
            return $result;
        }else{
            $uid = $user['uid'];
        }
        $setting = D('CreditSetting')->where(array('name'=>$action))->find();
        $score= D("CreditRecord")->getDalyScore($login);
        if($score['sumScore']+$setting['score']<=50) {
            $result['creditResult'] = $this->setUserCredit($user['uid'], $action);
            $arr = array();
            $arr['content'] = '新';
            $arr['url'] = '';
            $arr['rule'] = array(
            ...
            M('CreditRecord')->addCreditRecord($user['uid'], $login, $action, $arr);
        }
    }
}
