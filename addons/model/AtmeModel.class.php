...
class AtmeModel extends Model {
    ...
    /**
     * @param $content  获取at目标id
     * @return array|bool|返回新的一维数组
     */
    public function  getAtTargetUserIds($content){
        preg_match_all($this->_at_regex, $content, $matches);
        $unames = $matches[1];
        if(!isset($unames)||count($unames)<=0){
            return "";
        }
...
/*		if(in_array('atme',MailModel::$allowed)){
	...
	private function _saveAtme($uids, $row_id) {
        ...
        !empty($data) && $res = $this->execute('INSERT INTO '.$this->tablePrefix.'atme (`app`, `table`, `row_id`, `uid`) VALUES '.implode(',', $data));
