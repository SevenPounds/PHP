...
class MessageModel extends Model {
    ...
    protected $fields = array(0=>'message_id', 1=>'list_id', 2=>'from_uid', 3=>'content', 4=>"content_origin", 5=>'is_del', 6=>'mtime', '_pk'=>'message_id');
     ...
     */
    public function postMessage($data, $from_uid,$send_email = false,$node = 'new_message') {
        ...
        $config['content'] = strip_tags($data['content']);
        ...
        if($send_email){
        	$toUser = model('User')->getUserInfo($data['to']);
        	model('Mail')->send_email($toUser['email'], "资源审核不通过",$data['content'] );
        }
        return $data['list_id'];
    }
...
//             $member_map['member_uid'] = $member_uid;
    ...
    private function _addMessage($data, $from_uid) {
        ...
        $message["content_origin"] = $data["content_origin"];
        $message['is_del']   = 0;
