...
class NotifyModel extends Model {
	 ...
	 * @param integer $type 0:系统消息; 1:赞消息
	 * @return mix 更改失败返回false，更改成功返回消息ID
	 */
	public function setRead($uid, $appname = '',$type = 0) {
		$map['uid'] = $uid;
	 ...
	 * $type 区分是赞还是系统消息
	 */
	public function getUnreadCount($uid,$type = null){
		$map['uid'] = $uid;
		$map['is_read'] = 0;
		if(empty($type)){
			$map['type'] = 0;
	 ...
	 * @return void
	 */
	public function sendNotify($toUid, $node, $config, $from,$url) {
        //根据uid获取cyuid，uid指的是接受消息的用户
        $author = model ( 'User' )->getUserInfo ($toUid);
        $cyuid = $author['cyuid'];
        //调用OpenApi的发送消息服务
        ...
        //调用OpenApi的发送消息服务
        if($node == "digg"){
            $url = C('SPACE')."index.php?app=public&mod=Message&act=digg";
            $search = array(" ","　","\n","\r","\t");
            $replace = array("","","","","");
            $diggContent = str_replace($search,$replace,$config ['content']);
            $content = $config['uname']."赞了你的说说:".$diggContent;
            $epspClient->sendMessage($cyuid, "atme", $content,$url, $appKey);
        }
        if($node == "comment"){
            $url = C('SPACE')."index.php?app=public&mod=Comment&act=index";
            $commentContent = strip_tags($config['source_content']);
            $content = $config['name']."评论了你的说说:".$commentContent."@".$author['uname'].strip_tags($config['content']);
            $epspClient->sendMessage($cyuid, "atme", $content, $url, $appKey);
        }
        if($node == "new_message"){
            $url = C('SPACE')."index.php?app=public&mod=Message&act=index";
            $content = "您收到了".$config['name']."的一条私信:".strip_tags($config['content']);
            $epspClient->sendMessage($cyuid, "atme", $content, $url, $appKey);
        }
        if($node == "atme"){
            $url = C('SPACE')."index.php?app=public&mod=Mention&act=index";
            $atcontene = strip_tags($config['content']);
            $content = $config['name']."@你:".$atcontene;
            $epspClient->sendMessage($cyuid, "atme", $content, $url, $appKey);
        }
		...
		$data['type'] = 0;
		//赞消息type =1
		if(in_array($node, array('blog_comment_digg','digg','vote_comment_digg','research_comment_digg','pingke_comment_digg','agreeAnswer'))){
			$data['type'] = 1;
		}
		$data['node'] = $node;
