...
class RelatedUserModel extends Model {
	...
	public function getRelatedUserByType($type, $limit){
		...
		$this->_getExcludeUids(array($this->_uid));
		//过滤掉未激活，未审核，未初始化的用户
		$notin = model('User')->where('is_active=0 or is_audit=0 or is_init=0 and uid!=1')->field('uid')->findAll();
		$notinids = getSubByKey(  $notin , 'uid');
		$this->_getExcludeUids($notinids);
		$fids = $this->_user_follow->where('uid='.$this->_uid)->getAsFieldArray('fid');
		$this->_getExcludeUids($fids);
	...
	private function _getRelatedUserFromTag($num = 1 , $limit = 100) {
		...
		$sql = "SELECT `row_id`, `tag_id` FROM `{$this->tablePrefix}app_tag` AS a WHERE `tag_id` IN (".implode(',', $tagIds).") AND `row_id` NOT IN (".implode(',', $this->_exclude_uids).") AND `row_id` IN (SELECT uid FROM ts_user) group by `row_id` LIMIT {$limit}";
		$tagData = D()->query($sql);
		// Tag Hash数组
		$tagHash = array();
