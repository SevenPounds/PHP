...
class FollowGroupModel extends Model {
	...
	public function getDefaultGroupByPage($uid) {
		$uid = intval($uid);
		$data = $this->table('`'.$this->tablePrefix.'user_follow` AS a ')
					 ->field('a.fid,a.type')
					 ->where('a.uid = '.$uid.' AND a.type=0 AND NOT EXISTS( SELECT 1 from '.$this->tablePrefix.'user_follow_group_link where fid = a.fid and uid = a.uid)')
