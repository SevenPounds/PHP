...
class LogsModel extends Model {
	...
	public function cleanLogs($m) {
        ...
        if($dropTables) {
            return D('')->execute("DROP TABLE ".implode(',',$dropTables));
        } else {
        	return false;
        }
	}
    	...
    	if($this->where('ctime<='.$dayBeforeTime)->count()) {
    		...
    		foreach($findDate as $value) {
    			...
    			if(D('')->query("DESC $archiveTableName") == false) {
    				D('')->execute("CREATE TABLE $archiveTableName LIKE $logsTableName");
    			}
    			$querySql = "DATE_FORMAT(FROM_UNIXTIME(ctime),'%Y-%m') = '".$value[lists]."'";
    			$result = D('')->execute("INSERT INTO $archiveTableName SELECT * FROM $logsTableName WHERE $querySql");
    		}
