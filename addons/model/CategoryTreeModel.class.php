...
class CategoryTreeModel extends Model
{
	...
	public function getCategoryList($pid = -1, $field = '')
	{
		$map = array();
		$pid != -1 && $map['pid'] = $pid;
		empty($field) && $field = $this->_table.'_id, title, pid';
	...
	public function getCategoryById($id)
	{
		$result = array();
		if(!empty($id)) {
			$map[$this->_table.'_id'] = $id;
			$result = $this->_model->where($map)->find();
			$result['id'] = $result[$this->_table.'_id'];
			unset($result[$this->_table.'_id']);
		}
	...
	public function getNetworkList($pid = 0)
	{
		...
		// 全部分类树形结构
		$list = S('category_cache_'.$this->_table);
		if(!$list) {
			set_time_limit(0);
			$list = $this->_MakeTree($pid);
			S('category_cache_'.$this->_table, $list);
		}
	...
	private function _MakeTree($pid, $level = 0)
	{
		...
		if($result) {
			foreach($result as $key => $value) {
				$id = $value[$this->_table.'_id'];
				$list[$id]['id'] = $value[$this->_table.'_id'];
				$list[$id]['pid'] = $value['pid'];
				$list[$id]['title'] = $value['title'];
				$list[$id]['level'] = $level;
				$list[$id]['child'] = $this->_MakeTree($value[$this->_table.'_id'], $level + 1);
			}
		}
	...
	{
		S('category_cache_'.$this->_table, null);
	}
	...
	public function moveTreeCategory($id, $type)
	{
		...
		// 移动数据
		$pid = $this->_model->where($this->_table.'_id='.$id)->getField('pid');
		$data = $this->_model->field($this->_table.'_id, sort')->where('pid='.$pid)->order('sort ASC')->findAll();
		// 存储前后值
		$before = $in = $after = array();
		$keys = getSubByKey($data, $this->_table.'_id');
		...
		switch($type) {
			case 'up':
				if(!empty($before)) {
					$this->_model->where($this->_table.'_id='.$id)->setField('sort', $before['sort']);
					$this->_model->where($this->_table.'_id='.$before[$this->_table.'_id'])->setField('sort', $in['sort']);
					$result = true;
				}
				break;
			case 'down':
				if(!empty($after)) {
					$this->_model->where($this->_table.'_id='.$id)->setField('sort', $after['sort']);
					$this->_model->where($this->_table.'_id='.$after[$this->_table.'_id'])->setField('sort', $in['sort']);
					$result = true;
				}
				break;
		}
	...
	public function updateSort()
	{
		...
		foreach($pids as $pid) {
			$map['pid'] = $pid;
			$data = $this->_model->where($map)->order($this->_table.'_id ASC')->findAll();
			$sort = 1;
			foreach($data as $value) {
				$smap[$this->_table.'_id'] = $value[$this->_table.'_id'];
				$save['sort'] = $sort;
	...
	{
		// 判断名称是否重复
		$pid = $this->_model->where($this->_table.'_id='.$cid)->getField('pid');
		$isExist = $this->_model->where("pid={$pid} AND title='{$title}'")->count();
		if($isExist != 0 && empty($extra)) {
			return false;
		}
		// 更新分类操作
		$map[$this->_table.'_id'] = $cid;
		$data['title'] = $title;
		// 添加额外信息
		if(!empty($extra)) {
			$data = array_merge($data, $extra);
	...
	public function getCategoryHash($pid = -1)
	{
		$map = array();
		$pid != -1 && $map['pid'] = $pid;
		$data = $this->_model->where($map)->order('sort ASC')->getHashList($this->_table.'_id', 'title');
	...
	public function getCategoryAllHash ($pid = -1, $order = 'sort ASC') {
		$map = array();
		$pid != -1 && $map['pid'] = $pid;
		$list = $this->_model->where($map)->order($order)->getHashList($this->_table.'_id', '*');
		$data = array();
	...
	public function getNewPid() {
		$max = $this->_model->order('pid ASC')->getField('pid');
		return intval($max)-1;
	}
	...
	public function getPids() {
		return $this->_model->where('pid <= 0')->field('pid')->group('pid')->order('pid DESC')->findAll();
	}
}