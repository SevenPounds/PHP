...
class TagModel extends Model
{
	...
	protected $fields = array(0=>'tag_id',1=>'name',2=>'name_origin','_pk'=>'tag_id');
	 ...
	 * @param array $tagids 标签Ids -- by zhaoliang
	 * @return boolean 是否设置成功
	 */
	public function setAppTags($row_id, $tags, $max = 9, $tagids = array()) {
		...
		// 添加新设置
		$data = array();
		//modified by zhaoliang 2014/1/6
		if(count($tagids) > 0){
		...
		(false !== $res) && $res = $this->execute($sql);
		return false !== $res;
	}
	 ...
	 * @return integer 标签编号
	 */
	public function getTagId($name,$name_origin) {
		...
		if (!$result){
			$result = $this->add(array('name' => $name,'name_origin'=>$name_origin));
		}
		return $result;
	}
	...
	public function addTags($tags) {
		...
		foreach($tags as $k => $v) {
			$tags[$k] = mysql_real_escape_string(t(preg_replace('/^[\s　]+|[\s　]+$/', '', $tags[$k])));
			if (!$tags[$k]) {
				unset($tags[$k]);
			}
		}
		...
		$existing_tags = array_map('mysql_real_escape_string', $existing_tags);  // 数据安全转义
	 ...
	 * @version 2014-01-13
	 *
	 * @param string $app 所属app名
	 * @param string $table 所属表名
	 * @param int $limit 获得的个数
	 *
	 * @return array
	 */
	public function getHotTag($app, $table, $limit = 5) {
		if (empty($app) || empty($table)) return array();
		return $this->table($this->tablePrefix.'app_tag as t right join ' . $this->tablePrefix . 'tag as tg on t.tag_id = tg.tag_id')->field('t.tag_id, tg.name, count(*) as count')->where(array('t.app'=>$app, 't.table'=>$table))->group('t.tag_id')->order('count desc')->limit($limit)->select();
	}
}