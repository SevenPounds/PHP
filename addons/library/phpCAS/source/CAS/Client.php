...
//        return $this->_server['base_url'];
    ...
    public function getServerLoginURL($gateway=false,$renew=false)
    {
        ...
        if ( empty($this->_server['login_url']) ) {
            ...
            if ($this->_getNeedAttributes()) {
                $service_url = $this->_buildQueryUrl($service_url, 'from=ew');
            }
            if ($this->_getAppKey() != "") {
                $service_url = $this->_buildQueryUrl($service_url, 'appId='.$this->_getAppKey());
            }
            $this->_server['service_url'] = $service_url;
            $this->_server['login_url'] = $this->_buildQueryUrl($this->_getServerBaseURL().'login','service='.urlencode($service_url));
        }
...
//     */
    /**
     * @param $serverUrl SSO服务地址
     * @param $SSOLoginUrl 统一登录地址
     * @param bool $needAttribute 是否需要获取更多信息
...
//        	throw new CAS_TypeMismatchException($server_uri, '$server_uri', 'string');
        ...
        $this->_setNeedAttributes($needAttribute);
        $this->_setSSOLoginUrl($SSOLoginUrl);
        ...
        $this->_server['base_url'] = $serverUrl;
...
//        $this->_server['uri'] = $server_uri;
    ...
    private $_need_attributes = false;

    private function _setNeedAttributes($need_attributes) {
        $this->_need_attributes = $need_attributes;
    }

    private function _getNeedAttributes() {
        return $this->_need_attributes;
    }

    private $_sso_login_url;

    private function _setSSOLoginUrl($sso_login_url) {
        $this->_sso_login_url = $sso_login_url;
    }

    private function _getSSOLoginUrl() {
        return  $this->_sso_login_url;
    }

    private $_appKey;
    private function _setAppKey($appKey) {
        $this->_appKey = $appKey;
    }
    ...
    public function getAttributesWithoutValidation() {
     ...
     *
     * @param bool $redirectToLoginPage redirect to login page or not if the
     * user is not authenticated, default redirect to login page
     *
     ...
     */
    public function checkAuthentication($redirectToLoginPage)
    {
        phpCAS::traceBegin();
        $res = false;
        if ( $this->isAuthenticated() ) {
        ...
        phpCAS::traceEnd($res);
        if (isset($_SESSION["casreturn"])) {
            unset($_SESSION["casreturn"]);
//            $fullUrl = $this->getURL();
//            $fullUrl = preg_replace("/&amp;/i", "&", $fullUrl);

//            if (strpos($fullUrl, "nextpage=".$this->_getSSOLoginUrl()) != false) {
//                $_SESSION["force_logout"] = true;
////                if (strpos($fullUrl, "nextpage=") >= 0) {
////                    $nextpage = substr($fullUrl, strpos($fullUrl, "nextpage=")+9);
////                    $login_url = $this->_buildQueryUrl($this->_getSSOLoginUrl(), "nextpage=".$nextpage);
////                    $this->redirectToPage($login_url);
////                    return false;
////                }
////                $login_url = $this->_getSSOLoginUrl();
//                $loginUrl = preg_replace("/nextpage=/i", "nextpage1=", $fullUrl, 1);
//                $loginUrl = preg_replace("/nextpage1=.*?&nextpage=/", "nextpage=", $loginUrl);
//                $this->redirectToPage($loginUrl);
//                return false;
//            }
//            if (strpos($fullUrl, "nextpage=".$this->_getSSOLoginUrl()) != false) {
//                $_SESSION["force_logout"] = true;
//                $loginUrl = $this->_getSSOLoginUrl();
//                $this->redirectToPage($loginUrl);
//                return false;
//            }
            if ($redirectToLoginPage) {
                $loginUrl = $this->_buildQueryUrl($this->_getSSOLoginUrl(), "nextpage=".base64_encode($this->getURL()));
                $this->redirectToPage($loginUrl);
            }
            else {
        ...
        $SSOUrl = $this->getServerLoginURL(false, false);
        // modified by zhaoliang 03-07: 添加redirect=true parameter
        $SSOUrl = $this->_buildQueryUrl($SSOUrl, "sn=".$this->_generateSignature($this->_server['service_url']));
        $SSOUrl = $this->_buildQueryUrl($SSOUrl, "redirect=true");
        $_SESSION["casreturn"] = true;
        $this->redirectToPage($SSOUrl);
        return false;
        //return $res;
    }

    private static function _generateSignature($service) {
        $service = 'edc_'.$service.'_cde';
        return md5(base64_encode($service));
    }

    public static function redirectToPage($url)
    {
        $tourl = htmlspecialchars_decode($url);
    ...
    public function isAuthenticated($renew=false)
    {
        ...
        $isLegal = false;
        foreach($legalUrls as $legalUrl){
            if(strstr($this->getURL(),$legalUrl)){
                $isLegal = true; break;
            }
        }
        if(!$isLegal){
    ...
    public function redirectToCas($gateway=false,$renew=false)
    {
        ...
        // modified by zhaoliang 03-07: 添加redirect=true parameter
        //$cas_url = $this->_buildQueryUrl($cas_url, "redirect=true");
        session_write_close();
        $legalUrls = C('LEGAL_URL');
        $isLegal = false;
        foreach($legalUrls as $legalUrl){
            if(strstr($cas_url,$legalUrl)){
                $isLegal = true; break;
            }
        }
        if($isLegal){
    ...
    */
    ...
    public function validateCAS20(&$validate_url,&$text_response,&$tree_response, $renew=false)
    {
        ...
        $serviceResponse = new SimpleXMLElement($text_response, 0, FALSE, 'cas', TRUE);
        $users = (array)$serviceResponse->authenticationSuccess->user;
        if(!empty($users) && is_array($users)) {
            $this->_setUser($users[0]);
            $attributes = array();
            if ($serviceResponse->authenticationSuccess->attributes != null) {
...
//        return $result;
    ...
    private function _readExtraAttributesCas20($success_elements)
    {
        ...
        //
        $extra_attributes = array();
        //$serviceResponse = new SimpleXMLElement($success_elements, 0, FALSE, 'cas', TRUE);
        if ($success_elements->item(0)->getElementsByTagName("attributes")->length != 0) {
            $extra_attributes = (array)$success_elements->item(0)->getElementsByTagName("attributes");
        }
    ...
    */
    ...
    public function getURL()
    {
        ...
        if ( empty($this->_url) ) {
            ...
            $final_uri = preg_replace("/[\\?|&]ticket=.*/i", "", $final_uri);

            if ($this->_getNeedAttributes()) {
                $final_uri = $this->_buildQueryUrl($final_uri, 'from=ew');
            }
            if ($this->_getAppKey() != "") {
                $final_uri = $this->_buildQueryUrl($final_uri, 'appId='.$this->_getAppKey());
            }
            $final_uri = preg_replace("/&amp;/i", "&", $final_uri);
