...
class phpCAS
{
     ...
     * and phpCAS::setDebug()).
     */
    public static function client($serverUrl, $SSOLoginUrl, $needAttribute=false, $appKey="",  $changeSessionID = true) {
        ...
        try {
            self::$_PHPCAS_CLIENT = new CAS_Client(
                $serverUrl, $SSOLoginUrl, $needAttribute, $appKey, $changeSessionID
            );
        } catch (Exception $e) {
    ...
    */
     ...
     */
    public static function checkAuthentication($redirectToLoginPage=true)
    {
        phpCAS :: traceBegin();
        phpCAS::_validateClientExists();

        $auth = self::$_PHPCAS_CLIENT->checkAuthentication($redirectToLoginPage);
    ...
    /**
     * Answer attributes about the authenticated user.
     * From Session without validation
     * commonly need ensure the user has logined in CAS
     */
    public static function getAttributesWithoutValidation() {
        phpCAS::_validateClientExists();

        try {
            return self::$_PHPCAS_CLIENT->getAttributesWithoutValidation();
        } catch (Exception $e) {
            phpCAS :: error(get_class($e) . ': ' . $e->getMessage());
        }
    }
