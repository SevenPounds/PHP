...
class FollowGroupWidget extends Widget {
	...
	public function render($data) {
        ...
        // 组装数据
        if(isset($data['showLess']) && $data['showLess']){
            foreach($followGroupStatus as $k => $v) {
                $v['gid'] != 0 && $v['title'] = (strlen($v['title']) + mb_strlen($v['title'], 'UTF8')) / 2 > 8 ? getShort($v['title'], 4,'...') : $v['title'];
                $data['status'] = $v['title'];
                if(count($followGroupStatus)>1) {
                    $data['status'] .= '，...';
                    break;
                }
            }
    ...
    public function doaddGroup(){
        ...
        if($count >= 10) {
            $return = array('status'=>0,'data'=>'最多只能创建10个分组');
