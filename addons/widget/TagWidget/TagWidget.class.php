...
class TagWidget extends Widget{
	...
	public function render($data)
    {
        ...
        $var['nums'] = count($tags);
        if($var['tpl'] == 'tag_app'){
        	$type = $var['type'];//主题讨论 pid=-1、网上评课 pid=-2、网络调研 pid=-3、在线答疑 pid=-4
        	switch ($type){
        		case 'research':
        			$pid = -1;
        			break;
        	...
        	$var['categoryTree'] = model('CategoryTree')->setTable('user_category')->getNetworkList($pid);
        }else{
    ...
    public function getTagId()
    {
        ...
        $resultData = $this->sensitiveWord_svc->checkSensitiveWord($name);
        $resultData = json_decode($resultData, true);
        if ($resultData["Code"] != 0) {
            return;
        }
        $name = $resultData["Data"];
        $name_origin=t($_POST['name']);
    	// 其他相关参数
    	$appName = t($_REQUEST['appname']);
    	$appTable = t($_REQUEST['apptable']);
    	// $rowId = intval($_REQUEST['row_id']);
    	$tagInfo = model('Tag')->setAppName($appName)->setAppTable($appTable)->getTagId($name,$name_origin);
