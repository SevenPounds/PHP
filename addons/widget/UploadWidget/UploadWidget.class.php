...
class UploadWidget extends Widget{
    ...
    public function save(){
        ...
        $option['cid']=$_REQUEST['cid'];//记录班级附件的班级id by sjzhao20130922 调用upload widget 时直接通过urlquery传入参数即可
    	...
    	if($info['status']){
            ...
            $conmment = $this->user['uname']."于".date("Y-m-d H:i:s",time())."上传了“".$data['name']."”的附件";
            $logObj = $this->getLogObj(C("appType")["jyyy"]["code"],C("appId")[""]["code"],C("opType")["upload"]['code'],$data["attach_id"],C("location")["localServer"]["code"],"","",$data['name'],$this->mid,$this->user['login'],$this->user['uname'],$this->roleEnName,"",$conmment);
    ...
    public function down(){
        ...
        if(!$fileInfo){
            ...
            if($cloud->isOpen()){
                ...
                if(file_exists(UPLOAD_PATH.'/'.$filename)) {
                    model('Attach')->UpdatedownloadtimesById($aid);
        ...
        redirect($fileInfo->url);
        //Http::download($fileInfo->url);
