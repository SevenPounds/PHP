...
class FeedListWidget extends Widget {
	...
	public function render($data) {
        ...
        if(!empty($data['tpl'])){
        	$_feedListTpl = '_TimelineFeedList.html';
        }else{
        	$_feedListTpl = '_FeedList.html';
        }
        // 查询是否有话题ID
        if($var['topic_id']) {
        	$content = $this->getTopicData($var,$_feedListTpl);
        } else {
        	$content = $this->getData($var,$_feedListTpl);
        }
        // 查看是否有更多数据
    ...
    private function getData($var, $tpl = 'FeedList.html') {
        ...
        /* $fp = fopen("d://file.txt", "w");//文件被清空后再写入 
		$flag=fwrite($fp,json_encode($var)); */
    	switch($type) {
    		case 'following':// 我关注的
    			if(!empty($var['feed_key'])){
    				...
    				$list =  model('Feed')->getNEWFollowingFeed($where,$this->limitnums,'',$var['fgid']);
    			}
    			break;
    		case 'all'://所有的 --正在发生的
    			if(!empty($var['feed_key'])){
    		...
    		case 'space':	//用户个人空间
    			if(!empty($var['feed_key'])){
	    			...
	    			$map['is_del'] = 0;
	    			if(!empty($var['date'])){
	    				$dateStart = strtotime($var['date']);
	    				$firstday = date("Y-m-01 H:i:s",$dateStart);
	    				$dateEnd = strtotime("$firstday +1 month");
	    				$map['publish_time'] = array(array('gt',$dateStart),array('lt',$dateEnd));
	    				$limitnums = 21;
	    			}else{
	    				$limitnums = $this->limitnums;
	    			}
	    			
	    			// 用户说说页面，增加查询限制
	    			if($var['user_feed']){
	    				$var['feed_type'] = 'user_feed';
	    				$map['app'] = 'public';
	    			}
                    if($GLOBALS['ts']['mid'] != $GLOBALS['ts']['uid']) $map['is_audit'] = 1;
    				$list = model('Feed')->getUserList($map,$GLOBALS['ts']['uid'],  $var['feedApp'], $var['feed_type'],$limitnums);
    			}
    			break;
            case 'channel':
            ...
            case 'channelall':
                ...
                $list = D('ChannelFollow', 'channel')->getAllFollowingFeed($where, $this->limitnums, '' ,$var['fgid']);
                break;
            case 'reslib':
            	if(!empty($var['feed_key'])){
    				//关键字匹配 采用搜索引擎兼容函数搜索 后期可能会扩展为搜索引擎
    				...
    				$where =' (a.is_audit=1 OR a.is_audit=0 AND a.uid='.$GLOBALS['ts']['mid'].") AND a.is_del = 0 AND a.app='reslib'";
    				if($var['loadId'] > 0){ //非第一次
    					$where .=" AND a.feed_id < '".intval($var['loadId'])."'";
    				}
    				if(!empty($var['feed_type'])){
    			...
    			break;
            case 'school'://学校动态 ，我所在学校产生的动态
            		$userInfo = $GLOBALS['ts']['_cyuserdata'];
            		$classList = $userInfo['orglist']['school'];
            		...
            		if(!empty($var['feed_key'])){
            			...
            			$where =' (is_audit=1 OR is_audit=0 AND uid='.$GLOBALS['ts']['mid'].") AND is_del = 0 AND app='school'".$_where;
            			if($var['loadId'] > 0){ //非第一次
            				$where .=" AND feed_id < '".intval($var['loadId'])."'";
            			}
            			if(!empty($var['feed_type'])){
            		...
            		break;
            case 'class'://班级动态 ，我所在班级产生的动态
            			$userInfo = $GLOBALS['ts']['_cyuserdata'];
            			$classList = $userInfo['orglist']['class'];
            			foreach ($classList as $v){
            				if(2 == $v['type']){
            					$class[] = $v['id'];
            				}
            			...
            			if(!empty($var['feed_key'])){
            				...
            				$where =' (is_audit=1 OR is_audit=0 AND uid='.$GLOBALS['ts']['mid'].") AND is_del = 0 AND app='class'".$_where;
            				if($var['loadId'] > 0){ //非第一次
            					$where .=" AND feed_id < '".intval($var['loadId'])."'";
            				}
            				if(!empty($var['feed_type'])){
            ...
            foreach($var['data'] as &$v) {
            	switch ( $v['app'] ){
                    ...
                    case 'tipoff':
	                    ...
	                    break;
                    case 'blog':
                    	$v['from'] = getFromClient(0 , $v['app'] , '日志');
                    	break;
            		default:
    ...
    public function getTipoffDetail(){
	 ...
	 /**
	 *
	 *add 2015-08-26 生成个人主页更多页面
	 *
     */
	public function getMoreDynamics(){
		
		$var = array();
		$date = $_REQUEST["date"];

        if(isset ( $_REQUEST ['cyuid'] ) || !empty ( $_REQUEST ['cyuid'] )){
            $cyuid = t($_REQUEST['cyuid']);
            $uid = model ( 'User' )->getUidByCyuid($cyuid);
        }else{
            $uid = intval($_REQUEST["uid"]);
        }
		$firstday = date("Y-m-01 H:i:s",$date);
		$dateStart = strtotime($firstday);
		$dateEnd = strtotime("$firstday +1 month");
		$var['moreFeedList'] = D("Feed")->getMoreFeeds($uid, $dateStart, $dateEnd);
		$weiboSet = model('Xdata')->get('admin_Config:feed');
		$var['initNums'] = $weiboSet['weibo_nums'];
		$var['weibo_type'] = $weiboSet['weibo_type'];
		$var['weibo_premission'] = $weiboSet['weibo_premission'];
		$var['cancomment'] = isset($var['cancomment']) ? $var['cancomment'] : 1;
		$content = $this->renderFile ( dirname ( __FILE__ ) . "/" . "feed_lists.html", $var );
		// 输出数据
		//$return = array('status'=>1,'msg'=>L('PUBLIC_SUCCESS_LOAD'));
    	//$return['html'] = $content;
        //输出数据
